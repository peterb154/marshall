...
| **Opus** | The VOICE CODEC on the SRS wire (RFC 6716), 40 ms frames. Nothing to do with the Anthropic model of the same name — this document names Bedrock Sonnet and a dormant Haiku tier, so the word is genuinely ambiguous here. Encoded in `srs/tts.py`, decoded in `srs/client.py`. | SRS | no |
