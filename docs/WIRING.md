...
flowchart LR
  ...
  SRS -->|"UDP Opus frames plus sender GUID"| BR
  BR -->|"UDP Opus TX paced 40 ms, TCP 5002 Sync and roster"| SRS
...
**The deterministic core** (`src/marshall/atc/controller.py`) owns *separation*, and nothing else: who is on the board, what level each holds, who is in the letdown, who steps down when the bottom aircraft vacates, who is banished after two misses. It is blind — no radar, no telemetry, its whole world is what pilots reported plus a clock (`controller.py:1-34`) — and it is stateful, in the bridge process's RAM, so a bridge restart empties it. Around it sit three stateless helpers: `asr.py` (where he is on the approach, what heading regains course), `geometry.py` (the maths), `phases.py` (the legal phase transitions).

The seam is exactly one function: `separation_context()` at `agent_atc.py:1609`. It classifies the transcript, advances the `Controller`, and returns `(directive, stack)`. Those two strings are then injected into the agent's message under literal labels — `CONTROLLER (deterministic next step of the approach — voice its altitudes, headings and sequence exactly, add your radar read, never skip a leg):` and `SEPARATION (holding stack, one in the letdown):` at `agent_atc.py:3502-3507`. So the engine *decides* and the agent *speaks*. The same pattern is used for the flight roster (`FLIGHT (already decided from the roster and radar — SAY THIS...)`, `agent_atc.py:3489`) and for ASR guidance, on the same reasoning: who is in which flight and how far apart two aeroplanes are is the same class of fact as separation, and not the model's to invent.

**The one-sentence test.** Every altitude, heading, sequence number and "hold at" you hear should appear verbatim in a `CONTROLLER:` or `SEPARATION:` line printed in the bridge log immediately before the transmission (`agent_atc.py:3471-3474`) — if it does, the engine produced it and the agent only phrased it; if there is no such line and the number came out of the radio anyway, the agent invented it, and that is a prompt bug in `rules.md`, not an engine bug. The converse also matters: a directive that was printed but not spoken means the agent paraphrased away a leg, which is the failure the dry run (`tools/atc_dryrun.py`) exists to catch.

Two known consequences of the asymmetry, both real scars. The engine cannot see, so a false position report advances it before the agent can refuse it — the bridge reconciles this by checking a claimed station passage against the range of the track bound to that callsign, and by suppressing a vector when the directive says hold (`reconcile()`, `agent_atc.py:1391`). And the agent cannot retract: once the engine has acted, nothing on the radar side can undo it.
...
|---|---|---|---|
| **sim unit name** | `362nd_sockeye`, `Pony 1-2`, `Testbed 1-1`. The sim's own handle for one aeroplane. PRIMARY KEY of `tracks` (`director/tools/tracks.py:136`) and the value in `Identity.track`. | DCS. For AI flights pydcs is made to write `<group>-<n>` (`mission/build.py:219`); for a manned slot it is the slot's unit name. | never |
| **DCS player name** | `362nd_sockeye`. Arrives on the gRPC unit stream as `u.player_name`; stored as `tracks.player` (`tracks.py:158`). | the human's DCS profile | never |
...
| **radar label** | `tracks.label` = `player_name or callsign or name` (`tracks.py:163`). What prints at the head of a scope line. | derived, in the director | never |
| **DCS callsign** | `Enfield11`. The mission editor's AI callsign field. Deliberately left disagreeing with what the pilot calls himself — correlating the two from a position report is the machinery under test (`build.py:213-216`). | the .miz | never |
| **spoken callsign** | `"Pony one one"` — `callsign.Callsign.spoken` (`callsign.py:78`). What goes to Polly. Interpolating the canonical form instead makes Polly read the hyphen as "to", which the pilot hears as `Falcon 1-2-1` (see the comment at `agent_atc.py:88-100`). | `atc/callsign.py` | **yes** — this is the addressing form |
...
| **handle** | `sockeye`. `identity.handle()` drops every chunk of a name containing a digit (`identity.py:210-238`), so `Hoover 1-1-1` → `Hoover`. One person, survives a slot change and a callsign change. | derived from the unit / SRS name | never |
| **flight name** | `Apex`. One group, created by a lead saying so, gone when the group is. A *member number* (`Apex 1-2`) is not a third kind of name — hearing one is evidence the transmission is intra-flight and not addressed to us (`flights.py:1-34`). | `atc/flights.py` roster | **yes** |
...
| **board** | What the *deterministic engine* believes is flying — `Controller.board()` (`controller.py:359`), written to the flight recorder as `kind="board"` on every transmission (`agent_atc.py:3188`). Not the same as the scope, and the difference is the whole ghost problem. | bridge RAM | no |
...
| **plate** | The mission-specific prompt block generated by `atc/briefing.py:plate()` from `route.py` and PUT to `/prompts/plate` at bridge startup (`agent_atc.py:410`). Also, confusingly, the kneeboard chart of the same facts. If the controller says a number the chart does not show, suspect a stale plate in Postgres before suspecting drift. | generated from `core/route.py` | its contents are |
| **directive** | The deterministic engine's next-step string for this call, from `separation_context()` (`agent_atc.py:1609`), injected as `CONTROLLER (...)`. Empty for off-script calls, where the agent reasons freely. | `atc/controller.py` | its contents are, via the agent |
| **session** | `batumi-approach:<freq>` by default (`agent_atc.py:2490`), overridable as argv5. One per channel per mission instance — a shared context for the frequency, never a per-pilot chat. Keys the Postgres transcript, the hook queue, the identity bindings on `/radar`, and the director's per-session busy lock (`director/app.py:158`). | the bridge, at startup | no |

## Where to look first

| If this is wrong | Read |
|---|---|
| A number on the radio disagrees with the kneeboard, or the controller cites a fix that is not on the plate | `src/marshall/core/route.py`, then `atc/briefing.py:plate()` — and check that the startup line `pushed plate for <controller> to the director` (`agent_atc.py:411`) actually appeared |
| The wrong aeroplane was addressed, a callsign was invented, or a ghost took a level | `atc/identity.py` — `Registry.resolve` (`identity.py:405`) and the `(identity: ...)` line it prints (`agent_atc.py:3152`); then the `kind="board"` records in `build/logs/flight-<session>.jsonl` |
| Two aircraft were put on the same level, or a stack sequence looked wrong | `atc/controller.py` — the board and the queue. Check whether the engine was even engaged: it needs `n_contacts >= 2`, an existing stack, or `MARSHALL_SEP_ALWAYS=1` (`agent_atc.py:3195`) |
| The controller answered fluently but skipped a leg, paraphrased an altitude, or said something no directive contained | `director/prompts/rules.md` and the injected block at `agent_atc.py:3477-3512`; reproduce with `tools/atc_dryrun.py`, which uses the same message assembly with typed input |
| Silence — the call was heard and nothing came back | bridge stdout for a `..` suppression line or the readback/`answering` gate (`agent_atc.py:2566-2580`), then `docker compose logs` in `director/` for `session ... is still answering the previous call` (`director/app.py:171`), which drops rather than queues |
...
`__main__` is thirteen lines (`agent_atc.py:3654-3664`) and the argument order is positional, no flags:
...
- **`host`** — the SRS server (`_run_srs` passes it straight to `SRSClient`, `agent_atc.py:2533`). Note this is *not* read from `config.SRS_HOST`; the argument wins, always.
- **`freq_mhz`** — the *primary* channel only. `_run_srs` then adds every station frequency from the profile (`agent_atc.py:2517-2525`), so the bridge monitors 4 channels tonight, not one. See the startup line below.
- **`voice`** — the Polly voice for the *fallback* controller. Per-station voices come from the profile (`agent_atc.py:2497-2503`); this argument only covers a channel no station claims.
- **`session`** — defaults to `f"batumi-approach:{freq_mhz:.3f}"` (`agent_atc.py:2490`). This one string keys three things at once: the director's Postgres conversation, the `_atc_agents` cache entry, and the hook queue. **Changing it starts a brand-new controller with no memory of the sortie.**

The documented raw invocation (module docstring, `agent_atc.py:17`):

```sh
uv run --extra voice python -m marshall.atc.agent_atc --srs $SRS_HOST 132.0 Matthew
```

But **use the launcher instead**, `tools/bridge.py` — see the pkill trap below. Its defaults (`tools/bridge.py:42-45`) are the live configuration: freq `124.0`, voice `Matthew`, session `hooks`.
...
`start` is not fire-and-forget: it tails the log for up to 60 s and only returns 0 once it sees the word `monitoring` (`tools/bridge.py:115-134`). It also distinguishes "refused: another bridge holds the frequency" from "exited early". Log goes to `/tmp/marshall-bridge-live.log` (override with `MARSHALL_BRIDGE_LOG`).

**`BASE_URL` is hard-coded to `http://localhost:8000`** (`agent_atc.py:38`) with no env override. The bridge must run on the same host as the director, full stop.
...
| File | Read by | How |
|---|---|---|
| `/opt/marshall/.env` | the **bridge** (and every `tools/` script) | `config.py:45` — `_load_dotenv(REPO_ROOT / ".env")`, a hand-rolled parser, `os.environ.setdefault` so **an exported value always beats the file** |
| `/opt/marshall/director/.env` | the **director container** | `env_file:` in `docker-compose.yml:31-32`, injected by docker |

`config.py` resolves the repo root from `__file__` (`config.py:18`), so the bridge finds `.env` regardless of cwd. The live root `.env` holds only `SRS_HOST` and `SRS_EAM_PASSWORD`.
...
| Var | Default | Effect |
|---|---|---|
| `MARSHALL_FAST_TIER` | off | Routes routine phraseology to Haiku. Off deliberately — Haiku 4.5 doesn't hold the approach sequence (`agent_atc.py:44-50`) |
| `MARSHALL_SEP_ALWAYS` | off | Engage the separation engine with fewer than 2 radar contacts. The voice-only rehearsal needs this (`agent_atc.py:51-54`) |
| `MARSHALL_MISSION` | `default` | Partitions the `flights` table (`agent_atc.py:251`) |
| `MARSHALL_CALLSIGNS` | empty | Extra callsigns fed to the Whisper prompt and the flight-name set (`agent_atc.py:2155`, `:2391`) |
...
**`DCS_GRPC_ADDR` is the trap.** It is set *only* in `director/.env`, because the director container is its main consumer (`director/tools/dcs.py:34`). But the bridge reads it too, at `agent_atc.py:2462`, in `push_fixes` — and defaults it to `127.0.0.1:50051`, which is this LXC, where there is no sim. So a hand-started bridge fails the fix push on **every** start, and the controller ends up with only the field to compute bearings against. `tools/bridge.py:51-54` exists to fix exactly this: it greps `DCS_GRPC_ADDR=` out of `director/.env` and injects it. Any *other* hand-run script (`tools/spawn.py`, `tools/draw.py`, `tools/say.py`, `tools/survey_terrain.py`, `src/marshall/srs/pilot.py`) needs it exported by hand.

Director-side, `director/.env` currently pins `STRANDS_PG_MODEL_ID=us.anthropic.claude-sonnet-5` — which **overrides** the `claude-sonnet-4-5` default written into `app.py:64`. If you are wondering which model actually answered, read the container's own startup line, not the source.
...
|---|---|---|
| `loaded flight plan ... -> approach` (`agent_atc.py:397`) | The director answered `PUT /approaches`, `PUT /flightplans`, `GET /flightplan/active`. **The DB, not `route.py`, is now the profile in use** | `!! flight-plan bootstrap failed, using route.py:` (`:400`) — director down or unreachable. The bridge carries on with the `route.py` seed, so it does not crash; it just may disagree with what's in the DB |
| `pushed N named fixes (projected by the sim)` (`:404`) | gRPC reached the sim and `coord.LOtoLL` ran. `vector` can now give a real bearing to a steerpoint | `!! fix push failed, controller has the field only:` (`:406`). Almost always `DCS_GRPC_ADDR`. Not fatal — the approach needs none of it — but "distance to waypoint three" will come back as no-fix |
| `pushed plate for <controller> to the director` (`:411`) | `PUT /prompts/plate` succeeded; the agent's system prompt now carries tonight's numbers | `!! could not push plate:` — the agent is running on a **stale plate** from a previous sortie. This is the one that produces confidently-wrong altitudes |
| `agent ATC live as ... (voice ..., session ...)` (`:2537`) | SRS registration returned, `Controller` constructed, threads about to start. Echoes the session id — check it matches what you meant | — |
| `monitoring 139.000, 124.000, ...` (`:2539`) | The channel list. **This is what `tools/bridge.py start` waits for**, and it is the line to check when a pilot says "I called and nobody answered" — if his radio card frequency is not in this list, the call was inaudible from both ends (`:2511-2516`) |

Notice the frequency you passed on the command line (124.0) appears in the middle of the list, not first — the station list from the profile wins, and `freq_mhz` is only *inserted* if the profile doesn't already cover it (`:2524-2525`).

Three daemon threads start right after (`:2988-2990`): `engineering_radio`, `scheduler` (hooks + stale-board release, 2 s tick, `HOOK_POLL_SEC`), `asr_monitor` (the mile-call metronome, 4 s tick, `ASR_POLL_SEC = 4.0`). Then the main loop blocks on `client.recv_utterance` (`:2993`).
...
(`srs/client.py:211`). The TCP drain thread is the only thing that maps a voice packet's GUID to "Sockeye". When it dies the system keeps talking, just on worse evidence — which is why it prints loudly.
...
`track streamer started` (`app.py:138`) and `event stream started` (`app.py:141`) are the two gRPC consumers. They start regardless of whether the sim is up — an empty `tracks` table reads as *no contact*, never as confidently-wrong, so a dead streamer looks like clear skies. If radar is inexplicably empty, that is the pair to suspect, not the bridge.

Both `_FAST` and `_SMART` BedrockModels are constructed at import (`app.py:148-149`), which is why you see two `initializing` lines even with the fast tier off. `Found credentials in shared credentials file: ~/.aws/credentials` proves the `${HOME}/.aws` bind mount (`docker-compose.yml:41`) and `AWS_PROFILE` are working — no credentials means every `/atc` call 500s.
...
Everything on `:8000`, no auth (`auth_verifier` is not configured). Endpoints from `director/app.py` unless noted.

| Endpoint | Caller | Notes |
|---|---|---|
| `POST /atc` | bridge, `ask_agent` (`agent_atc.py:210`) | The hot path. Per-session `threading.Lock`, **non-blocking** — a second call while the agent is thinking is **dropped**, returns `{"busy": true, "response": ""}` (`app.py:161-184`). Silence on the radio with no error anywhere is this |
| `GET /radar?session_id=` | bridge, `fetch_radar` (`:197`); polled by both the scheduler and `asr_monitor` | Returns `{"picture": "..."}`, annotated with that session's identity bindings. Best-effort — a failure returns `""`, i.e. *no radar*, silently |
| `GET /hooks/due?session_id=` | bridge scheduler, every 2 s (`:2671`) | One-shot; a returned hook is already deleted |
| `PUT /prompts/{name}` | bridge at startup, for `plate` (`:410`); you, by hand, for `soul`/`rules` | See the caching scar below |
...
| `PUT /approaches/{name}`, `GET /flightplan/active` | bridge `load_and_push_plate` (`:385-390`) | Idempotent seed from `route.py`, then read back — the DB is truth |
| `PUT /fixes` | bridge `push_fixes` (`:2478`) | Lat/lon projected by the sim itself |
| `GET /fixes` | you, for checking the push landed | |
| `POST /flights/bind`, `/flights/{id}/agree`, `/flights/{id}/handoff` | bridge `flight_bind` / `flight_agree` / `flight_handoff` (`:319/330/341`) | **All three swallow their exceptions** (`:321/332/343`) — "the state store must never make him mute". Failures appear only as `!! flight bind failed:` in the bridge log |
| `GET /flights/airspace?callsign=` | bridge `leaving_my_airspace` (`:2065`) | Reports the handoff disagreement; does not decide |
| `GET /plans` | bridge `plan_labels` (`:2307`), `tools/plan_sweep.py`, and the kneeboard PLANS page (`kneeboard/plans.py:35`) | |
| `GET /flightplans` | bridge `filed_plans` (`:2342`) | |
| `GET /flights`, `DELETE /flights` | `tools/handoff_check.py:149`, and you between sorties | `DELETE` is how you forget last sortie's aircraft |
| `GET /health` | nothing automated | Registered by `make_app` (`api.py:194`) |
| `GET /events/departed` | **nobody** | Written for the bridge (`app.py:197-206`) but no caller exists in `src/` or `tools/`. The bridge currently drops stale aircraft off the board with `release_stale`'s 8-minute radar timeout instead (`agent_atc.py:433`, `STALE_BOARD_SEC`). Dangling |

**The prompt-cache scar, worth knowing before you edit a prompt.** `PUT /prompts/{name}` calls `invalidate_agents()`, which clears `make_app`'s cache — the one behind `/chat` (`api.py:170-171, 283`). It does **not** touch `app.py`'s module-level `_atc_agents` dict (`app.py:150`), which is the cache `/atc` uses. The system prompt is baked in at `build_agent` time (`app.py:98`). So: edit `soul` or `rules` live, and the running controller on an already-active session keeps the old text until the container restarts or you use a new `session_id`. The startup plate push works only because it happens *before* the first `/atc` call creates the agent for that session. Restart the bridge without restarting the director and a fresh plate silently does not take.

### Stopping the bridge — the pkill trap

Documented at length in `tools/bridge.py:1-23`. **Do not** run:
...
`tools/bridge.py running()` (`:58-74`) matches on the process list, explicitly excluding its own pid, its own process group, and anything containing `tools/bridge.py`. `stop()` sends SIGTERM, waits 15 s, then SIGKILL.
...
- **`claim_the_frequency()`** (`agent_atc.py:1921`) runs before `_run_srs` in `__main__` and takes an advisory `flock` on `build/bridge.lock`. Deliberately a flock, not a PID file — the kernel drops it however the process dies, so a crash can never leave a stale lock that blocks the next start. A refused start prints:
...
- **`OUR_STATIONS`** (`agent_atc.py:1815` — `{"Marshall", "Engineering", "Eartest"}`) makes the receive loop ignore anything transmitted by one of our own SRS names, so even if two do end up on the air they cannot talk to each other forever (`:3008-3011`, logs `(ignoring X -- that is one of ours, not a pilot)`).

Note the SRS client registers as `SRS_NAME = "Marshall"` (`:1803`), not as "Batumi Approach" — the roster shows the *service*, because one client is Center, Approach and Tower at once. Who is speaking is the voice and the callsign in the transmission.
...
A machine-readable copy of the same thing lands in `build/logs/flight-<session_id>.jsonl`, one JSON object per transmission (`record()`, `agent_atc.py:179`), specifically so a bad vector can be re-run against a real flight without flying it again.
...
The receive loop is the `while True` at the bottom of `src/marshall/atc/agent_atc.py` (`_run_srs`), with transcription in `srs/stt.py`, Opus encoding in `srs/tts.py`, the radio in `srs/client.py`, and `/radar` plus `/atc` served by `director/app.py`. The order that surprises: `fetch_radar` runs *before* anything looks at the words — and only when `profile.atc.radar` is set — because the GUID-to-client-name-to-unit-to-track chain has no microphone in it (`atc/identity.py`); the picture comes from the warm PostGIS track cache, falling back to a live gRPC scan only when that cache is cold (`director/tools/dcs.py`). `separation_context` does call a model, `bedrock_intent.classify`, but only to *name* the intent: `intents.dispatch` then advances `atc/controller.py`, and the queue and stack reach the agent as CONTROLLER and SEPARATION blocks it may phrase and never invent. `/atc` takes a per-session lock non-blocking, so a call arriving mid-answer is dropped rather than queued, which on the radio sounds like silence, not a late reply.
...
The second family of silences: the transmission passed every gate, reached Bedrock, and still nothing goes over the air. All of this is `for_voice`, `hush_a_second_talkdown`, `NO_CALL` and `channel_is_free` in `src/marshall/atc/agent_atc.py`, with `someone_is_talking` in `src/marshall/srs/client.py`. An HTTP failure inside `ask_agent` lands here too — the fallback reply is "Standby.", which `for_voice` blanks for want of a marker and `NO_CALL` would have dropped anyway, so an unreachable director sounds exactly like a controller who chose not to speak. Note also that a held metronome call is deliberately *not* recorded as issued, so it comes back on the next sweep — a delayed mile call is this path, not a dropped one.

The receive loop is the `while True:` at `src/marshall/atc/agent_atc.py:2992`. Everything below happens inside one iteration of it, on one thread, in the order written. Three other daemon threads (`engineering_radio`, `scheduler`, `asr_monitor`, started at `agent_atc.py:2988-2990`) transmit on their own schedules and are *not* part of this path — if the thing you heard was a mile call, a vector, a hook callback, or an engineering line, it did not come from here.
...
| `build/logs/flight-<session_id>.jsonl` | one JSON object per event, written by `record()` (`agent_atc.py:179`). `BUILD_DIR` defaults to `<repo>/build` (`config.py:54`) |
...
**1. Audio capture and end-of-utterance.** `client.recv_utterance(max_wait=3600)` (`srs/client.py:330`, called at `agent_atc.py:2993`) blocks on the UDP socket, decodes each SRS voice packet's 40 ms Opus frame to int16 PCM, and returns when **1.2 s of silence** has passed since the last packet (`silence=1.2`, `srs/client.py:330`). It also pulls two things off the packets: the transmit frequency out of the first freq block (`client.py:364-365`) and `last_sender_guid` from the trailing 22 bytes (`client.py:367`). It stamps `self.last_rx` on every packet, which is what `someone_is_talking()` (`client.py:378`) reads so the other three threads stay off the air.
...
**2. Whisper.** `stt.transcribe(model, pcm, prompt=whisper_vocabulary(profile))` (`agent_atc.py:2996`, `srs/stt.py:74`). faster-whisper `base.en`, CPU, int8. The prompt is rebuilt on **every** transmission by `whisper_vocabulary` (`agent_atc.py:2374`) from the squadron callsigns in `route.py`, `MARSHALL_CALLSIGNS`, every name any radio has ever claimed (`_transmitters`), the station names, the route fixes and the filed plan labels. An empty transcript `continue`s at `agent_atc.py:2998` with no log line — this is drop #2 in the table below and it is invisible.

**3. Who keyed the mic (free).** `srs = client.name_for(client.last_sender_guid)` (`agent_atc.py:3000`) — the SRS roster name, harvested off the TCP control channel (`client.py:215`).

**4. Guard: our own stations.** `agent_atc.py:3008`. If the SRS name is in `OUR_STATIONS` (`{"Marshall", "Engineering", "Eartest"}`, `agent_atc.py:1815`) or the GUID is our own, drop. This exists because a second bridge left running on the same frequency will hear this one, answer it, and the two will talk to each other forever.

**5. Radar, before identity.** `scope = fetch_radar(session_id) if radar_on else ""` (`agent_atc.py:3022`) → `GET /radar?session_id=...` on the director (`director/app.py:189`), which renders the PostGIS `tracks` table as prose annotated with this session's radar-identified callsigns. `radar_on` comes from `profile.atc.radar` (`agent_atc.py:2492`). This is best-effort: any failure returns `""` (`agent_atc.py:206`) and the whole rest of the loop then behaves like a no-radar mission. **A director that is down looks exactly like a procedural approach**, and nothing says so out loud.

**6. Identity.** Three calls, in this order:

- `claim = transmitter_callsign(guid, transcript)` (`agent_atc.py:3027`, defined at `:605`) — what the *words* claim, voted across the sortie by count-then-recency, filtered through `_plausible_callsign` (`:2246`) which demands a ≥3-letter alphabetic name that is either on the roster or in the first/last three words of the transmission. This is a **claim**, not an answer.
- `_ident = _identity.resolve(guid, srs, spoken=claim, scope=scope, plans=filed_plans(), roster=ctl.identified())` (`agent_atc.py:3028`, `identity.py:405`). The ladder, in order: **radar** via `unit_for_radio` (SRS name → sim unit, exact match, then handle match, then substring, refusing on ambiguity — `identity.py:251`); **by elimination** if exactly one manned unit on the scope is unclaimed (`identity.py:328`); **a filed plan** the claim matches; **the roster** of aircraft already admitted; otherwise a prior radar/plan resolution survives; otherwise nothing.
- `_who = identity.handle(_ident.track)` (`agent_atc.py:3035`) — the human out of the unit name, `362nd_sockeye` → `sockeye` (`identity.py:210`).

Logs: `  (identity: <why>)` at `agent_atc.py:3152`, printed **only when the authority is not radar**. Seeing `(identity: claimed 'Pony 1-1', already on the board)` for a pilot who should be on the scope means the radar chain broke upstream. `record(kind="pilot", ...)` at `:3169` carries `srs_name`, `claimed`, `authority`, `track`, `who`, `why` so a sortie can be scored afterwards.

**7. Guard: intra-flight talk.** `agent_atc.py:3042`. If the claim is a member designator of a flight that actually exists (`flights.is_intra_flight`, `flights.py:208`), it is the flight talking to itself and never lands in ATC. Logs `  .. <claim> is intra-flight, not for the controller`, records `kind="ship-to-ship"`, drops.

**8. Flight roster.** `agent_atc.py:3055-3126`, and only if `_who` is set (i.e. identity reached a track). Four parsers run over the transcript: `fl.parse_create`, `fl.parse_joining`, `fl.parse_leaving`, and the "he named a flight that does not exist" branch. Each writes a verdict into `_flight_say`. **The verdict is decided here, deterministically, from the roster and radar geometry (`miles_between`, `:519`) — the agent's only job is to say it**; the comment at `:3049` records the sortie where these were computed, recorded, and never voiced. `known` is then replaced with the flight name if he is in one (`:3146`, `_flights.speaking_as`).

**9. Book-keeping before any decision.** `_heard_on[known] = heard_hz` (`:3156`) — which channel he checked in on, later gating `may_be_vectored` (`:1590`). `_last_active_hz` (`:3160`) for hook callbacks. `count_contacts(scope)` (`:3162`, defined `:683`) counts *aircraft*, expanding "4 ships" on a formation line. Then the trace line:
...
(`agent_atc.py:3164`.) Two `record()` calls follow: `kind="pilot"` with the radar fix, and `kind="board"` with `ctl.board()` — the engine's state at that instant, deliberately adjacent to the words that produced it (`:3184-3189`).

**10. Does the deterministic engine engage?** `engaged = SEP_ALWAYS or n_contacts >= 2 or len(ctl.aircraft) >= 2` (`agent_atc.py:3195`). One ship, one contact, empty board → the engine is **off** and the reply is pure Sonnet on plate + radar. `MARSHALL_SEP_ALWAYS=1` forces it on for voice-only rehearsals.

**11. Canned reply.** `simple_response(transcript)` (`:3200`, defined `:89`) — radio check, or a closing call ("down and stopped", "clear of the active"). Only when **not engaged**. Transmits immediately, records `kind="atc/simple"`, logs `  ATC[simple] (0.0s): ...`, and `continue`s at `:3216`. No agent call, no engine.

**12. The deterministic controller.** `separation_context(ctl, transcript, scope, known, _ident.track)` (`:3218`, defined `:1609`), only if `engaged`. Inside, in order:

1. `bedrock_intent.classify(transcript)` — **a Sonnet call**, on the deterministic path (`bedrock_intent.py:26`: Sonnet, not Haiku, because the classifier decides which leg of the approach a call belongs to). This is why an engaged exchange is ~2 s slower than a single-ship one.
2. Implausible classified callsign → replaced with `known` (`:1651`).
3. Classified callsign that disagrees with the radio's binding → **overruled by the radio** (`:1694`), logging `  .. heard 'X', but this radio is Y`. Classified callsign with no radio binding and no radar tag → **discarded entirely** (`:1705`), logging `  .. 'X' is neither a radio we have identified nor a track on the scope`. This is the rule that keeps ghost aeroplanes out of the holding stack.
4. Claimed beacon passage contradicted by radar (>`OVERHEAD_NM` = 4.0 nm) → returns a `POSITION REJECTED` directive and the report **never reaches the engine** (`:1728`). Logs `  !! rejected: claims the beacon, radar shows 8.1 nm`. Only on a beacon approach; on a vectored one the guard is off, because the classifier files every position report as `REPORT_BEACON`.
5. `ctl.note_radar_contact` (`:1761`), `ctl.note_equipment` (`:1766`), `ctl.seen_on_final` (`:1777`).
6. `intents.dispatch(ctl, intent)` (`:1783`, `intents.py:270`) — the actual state transition. The engine's `Tx` objects are joined into `directive`; `_stack_summary` becomes `stack` (`:1797`), shown only with ≥2 aircraft not all resolved.

Logs: `  CONTROLLER: <directive>` (`:3472`) and `  SEPARATION: <stack>` (`:3474`), plus `record(kind="controller")`. Any exception here is swallowed with `  !! controller classify failed:` (`:1787`) — the transmission continues to the agent with no directive.

**13. Radar guidance.** `asr_context(profile, scope, known, _ident.track)` (`:3224`, defined `:1292`). Costs no model call, so it runs even for a single ship. Returns `""` if the profile is not `vectored`, if there is no fix, or if `is_on_the_ground` says he is down (`:1332`). Otherwise one `ASR: ...` line for the phase — vectoring, final, missed, or MAP. Logged as `  ASR: ...` (`:3262`) and recorded `kind="asr"`.

**14. The flight strip.** `flight_bind(...)` (`:3235`, `:316`) POSTs to `/flights/bind` on the director, joining GUID + SRS name + callsign + track into one row. **The track only goes in if radar actually produced a fix** (`:3246`). `flight_agree(...)` (`:3272`, `:325`) then writes what was agreed — phase, assigned altitude, procedure, runway — read off the engine, never parsed back out of English.

**15. Reconcile — one authority per aeroplane.** `reconcile(directive, stack, vectoring, _g)` (`:3256`, defined `:1391`), where `_g = asr.guide(...)` is the current geometry. Flying the missed → directive and stack suppressed. Established/final/MAP → any holding directive suppressed. Otherwise, if he has been told to hold → the vector is suppressed. Logs `  .. holding clearance suppressed: radar shows him established on the approach` etc. (`:3259`). Without this the agent was handed two confident contradictory instructions and said both.

**16. Engineering.** `agent_atc.py:3291-3370`, and note it is **after** the engine has already been advanced. `_opening` is the first six words; whoever is named first in it wins (`:3314-3318`). Release is tested before summons (`:3329`) so "thanks engineering" cannot re-open the line. If summoned or already on the line: the transcript is appended to `build/debug-notes.md`, logged as `  ENGINEERING [hh:mm:ss] who: text`, acknowledged in the Amy voice, and the loop `continue`s — **the controller never sees it**.

**17. Guard: out of the blue, no callsign.** `agent_atc.py:3393`. Fires when `not _said_who and not _open and known and not _flight_say`:
- `said_who` (`:828`) matches a numbered callsign *or* the closed set of flight names plus his own handle,
- `in_conversation` (`:1509`) is 90 s since this GUID last transmitted,
...
**18. Guard: ship-to-ship.** `addressed_to_another_aircraft(transcript, known, station_names)` (`:3406`, defined `:2107`). Reads the first 44 characters: a station name → ours; a callsign that is not the speaker's own → theirs. Refuses to decide if `known` is empty. Logs `  (ship-to-ship: X calling Y — not ours)`, records, drops.

**19. Guard: debug note.** `debug_note(transcript)` (`:3417`, defined `:76`). "debug", "debug log", "debug note" anywhere in the transmission. Writes to `build/debug-notes.md`, logs `  DEBUG NOTE [hh:mm:ss] ...`, and stays entirely off the air.

**20. Who is answering, and to whom he hands off.** `me = profile.station_on(on_mhz)` (`:3436`) — which controller owns the channel this arrived on. `handoff_on_the_event` (`:3443`, `:1982`) fires on the sim's own land/takeoff state; only if that is silent does the range rule `profile.handoff_from` run (`:3445`), with an explicit veto on approach→tower during a talkdown unless he is actually on the ground (`:3458`). Failing both, `leaving_my_airspace` (`:3468`, `:2017`) asks the director whether he has flown out.

**21. Message assembly.** `parts = []` at `:3475`, joined with newlines and POSTed as one string. In order:
...
| 3 | `STRIP: ...` | 3483 | the flights row had anything (`flight_strip`, `:347`) |
...
| 13 | `READ-BACK EXPECTED` | 3617 | `readback_due` (`:2214`), 150 s window armed when the last reply contained a squawk and a routing (`is_a_clearance`, `:2206`) |
| 14 | `READ-BACK CORRECT` | 3632 | `reads_back_what_we_said` (`:1167`) — his numbers match what we issued |
...
**24. Hush.** `hush_a_second_talkdown(reply, guide)` (`:2635`, defined `:1193`). If the geometry says final or MAP and the reply contains talkdown words, the whole reply is dropped, logging `  .. hushed the agent on final: the engine is flying the talkdown`. This exists because the agent's parallel mile calls did not merely duplicate — they held the metronome off the air and took the descent instructions with them.

**25. Nothing to say.** Empty reply, or one of `NO_CALL` (`:119`, `{"(no call)", "no call", "(none)", "standby."}`) → logs `  ATC[pilot/sonnet] (2.8s): (no call)` and returns (`:2638-2640`).

**26. TTS and transmit.** `voice_for(on_hz)` (`:2501`) picks the Polly voice belonging to the station that owns that channel. `Voice.frames(text)` (`srs/tts.py:112`) → `pronounce()` respellings → Polly PCM 16 kHz → 40 ms Opus frames. `client.transmit(frames, on_hz or freq_hz, AM)` (`:2650`, `srs/client.py:245`) sleeps 0.4 s to let the server register the UDP source, then paces frames at 40 ms real time — so a six-second reply holds `radio_lock` for six and a half seconds and everything else waits. **The reply goes out on the frequency the call arrived on**, which is by definition the beacon he is homing.
...
**27. After.** `hold_the_channel_for_a_readback()` (`:2655`) sets a 7 s window in which the unprompted threads stay silent. Back in the loop, `note_issued` has already recorded our numbers (`:2644`) for the next `reads_back_what_we_said`, and if the reply was a clearance, `_awaiting_readback[known]` is armed (`:3650`).
...
| 11 | `director/app.py:173` | agent still answering the previous call | `ATC[pilot/sonnet] (0.1s): (no call)` on the bridge; the real reason is in the **director** log |
| 12 | `agent_atc.py:162` | agent reply had no `RADIO:` marker | `ATC[pilot/sonnet] (3.1s): (no call)` |
...
- **Drops 8, 9 and 10 happen *after* the deterministic engine has already acted.** `separation_context` runs at `:3218`; the challenge, ship-to-ship and debug guards are at `:3393`, `:3406`, `:3417`. So with traffic on the frequency, a transmission that gets answered with "station calling, say your callsign" — or one that is silently filed as a debug note — has already advanced the holding stack, and `flight_bind`/`flight_agree` have already written to Postgres. If the stack moved and nobody was told, look here.
- **"Engineering, radio check" is answered by the controller, not by engineering.** `simple_response` matches `radio check` and `continue`s at `:3216`, well before the engineering block at `:3339`. Only true when the engine is not engaged.
- **`RADAR:` is fetched before identity is resolved, on purpose** (`:3013-3022`). The chain GUID → SRS name → sim unit → track has no microphone in it; the callsign is a label hung on the result. When you see the controller address someone by the wrong name but vector him correctly, that is a label problem (step 6's `_label`, `identity.py:357`), not an identity problem, and they are deliberately different failure classes.
- **A silent director degrades to a procedural controller with no announcement.** `fetch_radar` swallows every error (`:206`), so `scope=""`, `asr_context` returns `""`, `radar_fix` returns `None`, and the agent gets no RADAR line. The symptom is a controller who stops reading the scope; the cause is a container.
- **The intent classifier is a live Sonnet call inside the "deterministic" path** (`bedrock_intent.py:26`). It only runs when the engine is engaged, but when it does it adds latency to a path advertised as deterministic, and its failures are swallowed with `!! controller classify failed:` and no directive.
- **`is_on_the_ground` prefers the sim's event, and falls back to a geometry guess** (`:1964`). The fallback (alt < 200 ft and speed < 60 kt) is what runs after a director restart, because the event stream begins knowing nothing. A parked aeroplane reported as flying the missed approach is this fallback missing.
- **Formations collapse to one radar line, and the parser has to un-collapse it** (`identity.py:73-107`, `flatten_formation`). Every regex that reads a position looks for the first `N nm` on a line; on a formation line the first one is a wingman's offset. If a flight suddenly reads as three hundred yards off the runway, that is where to look.
...
|---|---|---|---|---|---|
| 1 | **sim unit name** — `362nd_sockeye`, `Pony 1-2`, `Testbed 1-1` | DCS. For AI, pydcs writes `<group>-<n>` — `mission/build.py:219`, `:269`, `:343`. For a manned slot it is the slot's unit name. | mission author (or the ME) | never | `tracks.name` (PRIMARY KEY, `director/tools/tracks.py:136`), `Identity.track`, `flight_state.track_name` |
| 2 | **DCS player name** — `362nd_sockeye` | the human's DCS profile; arrives on the gRPC unit stream as `u.player_name` | the pilot, in DCS settings | never | `tracks.player` (`tracks.py:158`), and it *becomes* the radar label for a manned contact (`tracks.py:163`) |
| 3 | **SRS client name** — `Sockeye` | the SRS client; with DCS running it is taken from the DCS export, so **it is normally the same string as #2** | the pilot, in SRS, if he runs standalone | never | `SRSClient.roster[guid]` (`srs/client.py:238`), `flight_state.srs_name`, `contacts.srs_name` |
| 4 | **SRS GUID** | generated per client, arrives free on every voice packet (`srs/client.py:367`) | nobody | never | `Registry.by_guid`, `_transmitters`, `_order`, `flight_state.srs_guid`, `engineering_line` |
| 5 | **radar label** — what the scope prints before the bracket | `label = u.player_name or u.callsign or u.name` (`tracks.py:163`), then de-collided by `_unique_labels` (`tracks.py:416`) | indirectly, by changing #2 | never | `contacts.track_label`, `identify.bindings_for()` |
| 6 | **DCS callsign** — `Enfield11` | the airframe's DCS callsign field. Left alone on purpose for AI (`mission/build.py:213-215`) — radar labels are *supposed* to disagree with what a pilot says | mission author | never | only as a fallback label at `tracks.py:163` |
| 7 | **spoken callsign** — `Pony 1-1`, `Falcon 1-1` | Whisper, then `callsign.extract_all` → `transmitter_callsign` (`agent_atc.py:605`) | the pilot, every time he keys the mic | **yes, constantly** | `Controller.aircraft` dict keys (`controller.py:259`), `contacts.callsign`, `_heard_on`, `_seen_at`, `radar_fix` |
| 8 | **handle** — `sockeye`, `Andre` | `identity.handle()` applied to the **track name** (`agent_atc.py:3035`) | nobody directly | yes, when he is not in a flight | `flights.Roster` membership, `_flights.of/speaking_as` |
...
`identity.py:32-42` states why this is the strongest link, with the measurement
...
— fetched from `GET /radar` (`agent_atc.py:3022` → `director/app.py:192` →
...
`units_on` (`identity.py:164`) parses both into `Unit(name, callsign, type,
manned, on_ground)`. Three things to know when a picture looks right but nobody
resolves:

- **The bracketed callsign is corroboration, never the key** (`identity.py:55-59`).
...
- **The colon is not always there.** A formation line has none, and assuming it
  ...
  pilot is about to ask to join a flight (`identity.py:61-72`). `_SCOPE_LINE`
  now accepts a colon *or* a lookahead to `IN FORMATION`.
- **`manned` is the elimination rung's whole input.** It comes from
  `u.player_name` being non-empty (`tracks.py:185`, rendered at `tracks.py:485`).
  If the picture never says `manned`, the guest path is dead and nothing else
  will tell you.

`flatten_formation` (`identity.py:93`) exists because every other regex in
...
three hundred yards off the runway. Call sites: `agent_atc.py:576, 744, 817,
888, 933`.

### `Registry.resolve` — the four rungs

`identity.py:405`. Called once per transmission at `agent_atc.py:3028` with
`(guid, srs_name, spoken=claim, scope, plans=filed_plans(), roster=ctl.identified())`.

| Rung | Code | `authority` | What it means |
|---|---|---|---|
| 1 | `identity.py:430` `unit_for_radio(srs_name, units)` | `radar` | The physical chain closed. No microphone anywhere. |
| 1b | `identity.py:436` `by_elimination` | **also `radar`** | One manned contact unclaimed, one unknown radio. Not a guess — elimination. |
| 2 | `identity.py:451` claim vs `plans` | `plan` | He said a name that matches a **filed strip**. |
| 3 | `identity.py:458` claim vs `roster` | `roster` | He said a name already on the board — borrowed authority. |
| 4 | `identity.py:469` prior, if it was `radar` or `plan` | unchanged | Not a new authority; the one he already had. Survives a clipped call. |
| — | `identity.py:472` | `""` | "Say again." |

Note what is **absent**: a rung where a well-formed callsign matching nothing
becomes an aeroplane. `identity.py:421-422` — "That rung is the bug." 846
recorded transmissions bound one radio to 37 distinct names, of which ten were
aeroplanes.

Rung 2 reads `/flightplans`, **not** `/flights` (`agent_atc.py:2342-2360`). The
...
transmission. Rung 3's input is `ctl.identified()` (`controller.py:340`), which
returns only `radar_identified` aircraft — so a ghost cannot vouch for itself on
the second hearing.

Rung 1 is **re-run every transmission**, never cached (`identity.py:427-429`).
That is the answer to #38's "how do I reset it without an engineer": there is
nothing durable to reset. `Registry.forget` (`identity.py:317`) exists for the
same reason — but note **it has no caller anywhere in the repo**; only
`tests/test_identity.py:161` exercises it. The reset story rests entirely on
re-derivation.

### `authority` in the recorder

`Identity.authority` is the point of the whole module (`identity.py:150-153`). It
is written to the flight recorder on every pilot line (`agent_atc.py:3177`) and
...
`agent_atc.py:3148-3152`. A regular pilot showing `plan` or `roster` on every
...
**(a) is a real, silent failure mode.** `srs/client.py:88-104`: `create_connection`'s
...
(`tests/test_identity.py:288`): `unit_for_radio("Sockeye", [Unit("Testbed 1-1")])`
must be `None`.
...
radar"`. And `identity_watch.py:80` prints `why` **only when authority is not
...
`identity.py:251`. Three passes, in this order, and the order is a scar:

1. **Exact on `_key()`** (`:280`). `_key` strips everything non-alphanumeric and
   lowercases (`:241`), so `362nd_sockeye` → `362ndsockeye`. With DCS running the
   SRS name and the radar label are the *same string*, so this is the normal case.
2. **Exact on the handle** (`:290`). `Andre` vs `362nd Andre` — same person,
   neither contains the other once a squadron tag is in the way.
3. **Substring either direction** (`:298`).
...
one whose name contains it (`identity.py:274-279`, `tests/test_identity.py:410`).

**Ambiguity is refused, never tie-broken**, at every pass (`:282-284`, `:295-296`,
`:299`). The reasoning is at `identity.py:258-262`: *a wrong identity is worse
than none, because none produces "say again" and wrong produces a clearance for
the wrong aeroplane*. `flights.near_name` (`flights.py:107`) and the agent's own
radar-ID rule (`director/tools/identify.py:15-17`) follow the same rule.

Names shorter than 3 characters after `_key` return `None` — "too short to be
evidence of anything" (`:265`).

`by_elimination` (`identity.py:328`) refuses the moment it becomes a choice: it
...
`identity.py:210`. **The rule is "drop any chunk with a digit in it"**, split on
...
pilot calling himself `Viper2` is still somebody (`:233-234`).
...
(`identity.py:216-223`, `flights.py:1-33`, [ARCH-4]/#42).

`_who` is `handle(_ident.track)` — the handle of the **sim unit name**, not of the
SRS name (`agent_atc.py:3035`). **Consequence worth holding on to: `_who` is empty
...
hid a live `AttributeError` for weeks (`agent_atc.py:3128-3143`): the crashing
...
`identity.py:357`. The asymmetry is deliberate: **the track is what gets
...
- `spoken` is not one transmission's guess. It arrives pre-voted by
  `transmitter_callsign` (`agent_atc.py:605`), which counts how often a radio has
  ...
  one` has become specific, not changed its mind (`:638-656`).
- **A second guard here was a real outage** (`identity.py:371-385`). It refused
...
- **The last-resort chain ends at `handle(u.name)`, not `u.name`**
  (`identity.py:393-402`). Reaching the unit name put a squadron tag on the air:
...
name** (`agent_atc.py:3146-3147`).

### Where the label is still a key — the residual risk

`Controller.aircraft` is `dict[str, Aircraft]` keyed on the **callsign string**
(`controller.py:259`, `:586`). So is `_heard_on`, `_seen_at` (via `_key_name`,
`agent_atc.py:849`), and `radar_fix`. The mitigation is that guidance is looked up
**by track first**:

```python
_fix = (radar_fix_by_track(scope, _ident.track, profile)
        or radar_fix(scope, known, profile))       # agent_atc.py:3167
```
...
Georgia Center eight times in one approach (`agent_atc.py:3239-3246`).
...
claim matches a filed plan, the loop at `identity.py:451` overwrites `by_guid`
...
fallback at `:469` is only reached when rungs 2 and 3 both miss.
...
and it expires after `BINDING_TTL_SEC = 2h` (`identify.py:96`) — added after a tag
...
1. **`radio=`** — a six-hex-character stub means the SRS roster is broken
   (`srs/client.py:88`), and *everything* below it is downstream of that.
...
produced it, `agent_atc.py:3184-3189`), and RADIO. It prints explicitly:
...
purpose (`whos_who.py:103-107`) — "no snapshot yet" and "the engine is tracking
...
(`agent_atc.py:3169-3183`). Both of the earlier weak paths are replayable from an
...
- "**Station calling, say your callsign**" from a man who named himself → either
  `said_who` (`tests/test_identity.py:630`: a bare handle used to fail the
...
- A **flight verdict never voiced** → the verdicts are computed deterministically
  and pushed to the agent as `FLIGHT (already decided ...)` (`agent_atc.py:3489`).
  If the pilot heard nothing, look for `_flight_say` being empty because `_who`
  was empty because there was no track.
- **Two controllers arguing** → `OUR_STATIONS` (`agent_atc.py:1815`) and the
  self-GUID check at `:3008`. A second bridge on the same frequency is trivially
...
Two files. `src/marshall/atc/flights.py` (383 lines, no imports beyond `re`/`time`/`dataclasses`) holds the whole model and is pure state; `src/marshall/atc/agent_atc.py:3049-3142` is the only caller — one block inside the SRS receive loop that reads the transcript, mutates the roster, and produces a single string, `_flight_say`, which is later handed to the agent as a directive it must voice verbatim.
...
The handle comes from `identity.handle()` (`identity.py:210-238`) — drop every chunk of the DCS unit name that contains a digit, so `362nd_sockeye` → `sockeye` and `Hoover 1-1-1` → `Hoover`. It is never spoken by a pilot and never transcribed, which is why it can be trusted.
...
class Flight:
    ...
    formed_at: float = 0.0
```
`flights.py:66-75`. The lead is also in `members` — `create` appends him (`flights.py:260`). `Flight.has` and every other comparison inside `flights.py` go through `_same()` (`flights.py:78`), which is case- and whitespace-insensitive.

`Roster` (`flights.py:223-360`) is a single dict `name -> Flight` plus six methods. It lives as one process global:

```python
_flights = fl.Roster()          # agent_atc.py:508
```
...
Do not confuse `_flights` with `_flight` (singular, `agent_atc.py:3235`). `flight_bind` / `flight_agree` / `flight_handoff` / `flight_strip` (`agent_atc.py:316-370`) are the **director's** flight-strip board — an HTTP row in Postgres holding clearance, assigned altitude and procedure. Completely separate mechanism, unfortunately similar name.
...
There is **no size and no pending state**. An earlier version had the lead declare a count so the flight was not treated as one until that many joined; `flights.py:243-251` explains why it went — every member joins on his own radio, so every member has been heard by construction, and a count is one more thing Whisper can get wrong for no benefit. `parse_create` reads a name and discards anything else (`flights.py:131-141`).

### The one-mile rule, and where the distance comes from

```python
JOIN_NM = 1.0                   # flights.py:64
```

Formation distance, "because joining is the moment the controller stops separating him, so it had better be true" (`flights.py:62-63`). The test is `if miles_from_lead > JOIN_NM` (`flights.py:304`) — exactly 1.0 passes.
...
```
`agent_atc.py:3081-3084`.

- `_track_of(scope, handle)` (`agent_atc.py:511-516`) walks `identity.units_on(scope)` and returns the scope label whose `handle()` is the lead's.
- `miles_between` (`agent_atc.py:519-560`) works purely off the radar text the director returned this cycle. `_scope_geometry` (`agent_atc.py:563-602`) builds two dicts because a formation genuinely has two kinds of member: a lead with range+radial from the field, and wingmen printed only as `0.3 nm` off him. So there are four cases — both absolute (hypotenuse), one is the other's wingman (return his offset), both wingmen of the same lead (sum of offsets), wingman vs. stranger (lead's distance plus the offset). The last two are **upper bounds on purpose**: over-estimating costs a false refusal he can recover from by saying it again; under-estimating puts a man in a formation radar cannot confirm (`agent_atc.py:539-543`).
- When the picture prints a formation with no offset figure, `offset[k]` is stored as `None`, **not zero** (`agent_atc.py:592-601`). Zero would read as "they are touching" when what is actually known is only "inside the formation threshold", which is twice the join radius.
...
Two things to expect from that second string. The distance is formatted `:.0f`, so a refusal at 1.4 nm reads *"radar shows you 1 miles from Apex — you must be within 1 mile to join"*, which sounds like nonsense on the radio and is not. And `radar_on` gates the whole scope fetch (`agent_atc.py:3022`); with radar off, every join is the "does not show you both" refusal.

On the ground the rule needs no special case — parked aeroplanes are yards apart (`tests/test_flights.py:421-455`), so a flight formed at the ramp gets one clearance and one squawk, which is what a formation gets in reality.
...
if claim and fl.is_intra_flight(claim, _flights.names()):
    ...
    continue
```
`agent_atc.py:3042-3047`. Note it is applied to `claim` — the voted callsign from `transmitter_callsign` — not to the raw transcript, so it fires on the callsign the sortie-long vote extracted.

This is the design's best trick, stated at `flights.py:29-33`: a member number is evidence the transmission is **not addressed to the controller**, so the hardest case to resolve becomes a case there is no need to resolve. The "only for a flight we actually know" guard matters — an unknown name in that shape is somebody flying under the old callsign scheme, and refusing to hear him would be worse than the noise (`flights.py:213-215`).
...
All three are called on the raw `transcript`, in this order, each on the same transmission (`agent_atc.py:3060`, `3079`, `3108`).

| parser | regex | line |
|---|---|---|
| `parse_create` | `creat\w*|form\w*|establish\w*` + a name | `flights.py:45-47` |
| `parse_joining` | `join(ing|s|ed)?` | `flights.py:50` |
| `parse_leaving` | `separat\w*|break\w*\s+(out|away|off)|detach\w*|depart\w*\s+the\s+flight|leav\w*` | `flights.py:54-56` |

`parse_joining` returns a **pair**: `(known flight, what he said)`. The first is only non-empty when the word resolves to a flight that exists; the second exists purely so a refusal can echo his word back. `flights.py:153-158` is explicit about why that asymmetry is safe — "a mis-heard name in an apology costs nothing, and the same mis-hearing used to put him in a formation costs him his separation."

He does not have to say who he is. The identity ladder already knows which aeroplane is transmitting; the only thing to find in the words is which flight he means (`flights.py:146-151`).
...
`near_name(said, flight_names)` (`flights.py:96-128`):

- shorter than 3 characters → `""`
- exact match (case-insensitive) wins outright
- otherwise Levenshtein (`_distance`, `flights.py:82-93`) with a threshold of `max(1, len(n) // 4)` — one edit per four characters
- **more than one candidate inside the threshold → `""`**, refused rather than tie-broken, the same rule as `identity.unit_for_radio`

Whisper turned "Apex" into "Abex" in rehearsal and a wingman formating half a mile off his lead was refused the flight he was already in (`flights.py:102-104`, `173-176`). With one flight airborne, "Abex" can only have meant that one — but matching against unbounded English would be a guess, while matching against the two flights that exist is a lookup. That distinction is the whole design.

It is **only ever called on the word in the grammatical slot** — after "joining" or "leaving" — never scanned across the transmission (`flights.py:111-115`). A fuzzy match anywhere in a sentence would eventually catch an ordinary word, and flight names are short enough to sit one edit from real English. If you see a join fire on a transmission that only mentioned a flight in passing, that constraint has been broken.

### Losing a member, and losing the lead

`Roster.leaves(handle)` (`flights.py:314-348`):
...
`lead_lost_call(flight, lead, survivors)` (`flights.py:363-382`) builds the announcement, and the ordering is deliberate — fact, consequence, question:
...
Flight-recorder kinds written to `build/logs/flight-<session>.jsonl` (`record`, `agent_atc.py:180`): `flight/created`, `flight/joined`, `flight/refused`, `flight/left`, `flight/dissolved`, and `ship-to-ship` for the intra-flight case. The join records `miles=round(_gap or 0, 1)` (`agent_atc.py:3088`) — so a `flight/refused` with `miles: 0.0` means the distance was `None`, not that they were on top of each other.
...
The entire block is inside `if _who:` (`agent_atc.py:3059`), and:

```python
_who = identity.handle(_ident.track) if _ident.track else ""     # agent_atc.py:3035
```

`_who` requires a **track**, which means `Registry.resolve` returned `authority == "radar"` — rung 1 of the ladder (`identity.py:405-448`), the physical chain radio GUID → SRS client name → DCS unit → track, with no microphone in it. Rung 2 (a claim matched against a filed plan) and rung 3 (a claim matched against somebody already on the board) both set `callsign` and leave `track` empty (`identity.py:453`, `461`), so:
...
```
`agent_atc.py:3148-3152`. So `(identity: claimed 'Pony 1-1', already on the board)` in the log immediately before a flight call that did nothing is the whole explanation. `(identity: ...)` printing at all when a pilot should be on radar means something upstream — the radar fetch, the SRS/DCS name match — has broken, and flights are one of the things that quietly stop working when it does.
...
`_flight_say` is not transmitted directly. It is appended to the agent's message as a directive (`agent_atc.py:3489-3501`):
...
```python
if not _said_who and not _open and known and not _flight_say:   # agent_atc.py:3393
...
`controller.py` is blind (`controller.py:1-34`). It has no radar, no telemetry, no connection to the sim. Everything it believes came out of somebody's mouth, through Whisper, through the intent classifier. `asr.py` is the opposite: it sees, and remembers nothing. Every call to `asr.guide()` re-derives the whole situation from one radar Position (`asr.py:453`). Almost every confusing thing you will hear on the radio comes from that asymmetry, and from the bridge's attempts to bridge it.
...
|---|---|---|
| "Falcon one one, four miles from the runway, on course, descend to one thousand two hundred" | **computed** — `asr_call()` `agent_atc.py:1222`, no model involved | `  ATC[asr] ...` |
| "Falcon one one, turn right heading one three zero, maintain three thousand" | **computed** — `vector_call()` `agent_atc.py:1266` | `  ATC[vec] ...` |
| "Falcon one one, Batumi Tower, welcome. Exit the runway when able, taxi to parking." | **computed** — `Controller.report_down()` `controller.py:879` | `  ATC[down] ...` |
...
`CONTROLLER:` is the deterministic engine's directive (`agent_atc.py:3472`), `SEPARATION:` is the stack summary (`_stack_summary`, `agent_atc.py:701`), `ASR:` is the geometry (`asr_context`, `agent_atc.py:1292`). All three are injected into the agent's prompt as labelled blocks (`agent_atc.py:3503-3511`) with instructions to *voice the numbers exactly*. So a heading the agent spoke may still have been computed — check whether the number in `ATC[pilot/sonnet]` matches the number in the `CONTROLLER:`/`ASR:` line above it. If it does not, that is the agent paraphrasing, which is the failure mode `tools/atc_dryrun.py` exists to catch cheaply.

### When the deterministic engine engages at all

One line, `agent_atc.py:3195`:

```python
engaged = SEP_ALWAYS or n_contacts >= 2 or len(ctl.aircraft) >= 2
```

- `SEP_ALWAYS` is `MARSHALL_SEP_ALWAYS=1` (`agent_atc.py:54`) — for the voice-only rehearsal, which has no radar tracks at all.
- `n_contacts` is `count_contacts(scope)` (`agent_atc.py:683`), which counts **aircraft, not lines**: radar collapses a formation into one line with "— 4 ships —", so counting lines would switch the engine off for the arrival that most needs sequencing.
...
The ASR half is gated differently and separately: `asr_context()` runs for a single ship too, because it costs no model call (`agent_atc.py:1292`, and it returns `""` immediately unless `profile.vectored`). The metronome thread has its own gate, `radar_on and profile.vectored` (`agent_atc.py:2750`).

So: **a stale board entry turns a single-ship approach into a sequencing problem.** That is the scar behind `release_stale()` (`agent_atc.py:433`) and `Controller.release()` (`controller.py:293`) — a pilot who flew as Falcon 1-1, left the slot and came back as Pony 1-1 was held at five thousand behind his own former self. Symptom in the log:
...
`Controller.aircraft` is `dict[callsign -> Aircraft]` (`controller.py:259`). `Aircraft` (`controller.py:60-119`) carries phase, `assigned_ft`, `approaches`, `members[]`, `map_t`, `on_visual`, `kit`, and `radar_identified`.

**Getting on.** Only through `separation_context()` (`agent_atc.py:1609`) → `intents.dispatch()` (`intents.py:270`) → one of `check_in` / `report_beacon` / `report_missed` / `report_landed` / `request_approach` / `request_breakup` / `request_visual`. Every one of those calls `Controller.get()` (`controller.py:502`), which *creates on demand* — which is exactly how ghosts were minted. Three defences now sit in front of it, all in `separation_context`:

1. `_plausible_callsign` — a callsign needs a NAME, not just a number (`agent_atc.py:1651`). Log: `  .. ignoring 'Maintained 2' -- that is not a callsign`.
2. **The radio, not the words** — if this SRS GUID already answers to a callsign, that callsign overrules the classifier (`agent_atc.py:1694`). Log: `  .. heard 'Hammer 1-3', but this radio is Hammer 1-1`.
3. **Unidentified callers reach the engine not at all** — if we do not know whose radio it is and the name is not a tagged track on the scope, the intent's callsign is blanked (`agent_atc.py:1698-1709`). Log: `  .. 'Left 3-8' is neither a radio we have identified nor a track on the scope; the engine will not be told about it`.

**Getting off.** Three ways: `report_landed`/`report_down` set `Phase.LANDED`; `release()` removes the entry outright (`controller.py:293`), driven by `release_stale()` on the 2-second scheduler tick (`agent_atc.py:2663`, `HOOK_POLL_SEC = 2.0`); and `_break_up()` pops a flight and replaces it with nothing, remembering the members in `_broken_up` (`controller.py:753-755`).

**Reading the board.** `Controller.board()` (`controller.py:359`) is written to the flight recorder on *every* pilot transmission (`agent_atc.py:3188`, `kind="board"`), so a ghost is timestamped and adjacent to the words that created it. `tools/replay.py` is where you read it back. `identified()` (`controller.py:340`) is deliberately narrower than `aircraft.keys()` — only radar-vouched aircraft may corroborate anybody else's identity, or a mis-hearing gets more convincing each time it repeats.

### Formations

A joined flight is ONE `Aircraft` with `members[]` (`controller.py:60-70`), holding one stack slot. `_resolve()` (`controller.py:475`) is what makes any member's transmission count as the flight talking — which is realism *and* the best defence against Whisper hearing "one two" for "one one".

Break-up (`_break_up`, `controller.py:723`) now does exactly one thing: **the flight stops existing.** No levels are assigned to the members. Each is an ordinary arrival from that moment and must check in and ask for the approach himself. The old visual-separation tri-state and the "can you maintain visual separation between your aircraft?" question are **gone** — separation inside a formation is the lead's, per the quoted comment at `controller.py:735-752`. Note that `docs/DESIGN.md:122-136` still describes the old tri-state behaviour; the code is the newer authority.

Break-up is triggered by arrival at the fix (`report_beacon`, `controller.py:783`), by `request_approach` (`controller.py:961`), by `request_visual` (`controller.py:921`), or by lead asking (`request_breakup`).

After break-up the flight NAME means nobody. `ambiguous_after_breakup()` (`controller.py:506`) returns True and `say_again_who()` (`controller.py:531`) asks rather than guessing — if you hear "Pony one flight, you are broken up for individual approaches — say your callsign. I have Pony one one, Pony one two.", that is this, and it is computed.

### The holding stack and one-in-the-letdown

The levels are **generated, not listed** — `ApproachProfile.stack_ft` (`route.py:1060`) is `range(hold_base_ft, hold_top_ft+1, hold_step_ft)`. On a vectored approach the base is raised above the cloud tops (`route.py:1076-1079`), because "hold present position" is only a real instruction if the pilot can see. That is why the ASR stack at Batumi starts at 12,000 and not 4,000.

The rules, all in `controller.py`:

| rule | where |
|---|---|
| enter at the top — lowest free slot above the current holders | `_free_slot()` `controller.py:563` |
| step down on vacate — when the bottom is cleared, everyone drops one level | `_step_down()` `controller.py:1053`, called from `_try_clear()` `controller.py:1050` |
| one in the letdown — `_letdown` holds exactly one callsign | `controller.py:262`, `_try_clear()` `controller.py:1031` |
| missed → front of the line | `_next_up()` `controller.py:1022` prefers `Phase.MISSED` |
| repeat miss (≥2) → banished to the outer hold at `top_ft` | `_do_missed()` `controller.py:831`, `MAX_APPROACHES = 2` |
| silent letdown → released after 12 min, do not deadlock | `tick()` `controller.py:1093`, `CLEARANCE_TIMEOUT_SEC` |
| quiet holder prompted after 5 min, **one per tick** | `tick()` `controller.py:1104`, `REPORT_OVERDUE_SEC` |

`_step_down()` steps **levels, not aircraft** (`controller.py:1061`) so that a shared level does not get silently laddered apart.

Two behaviours here are surprising and deliberate:

- **`request_approach` enters him in the stack silently.** `controller.py:1000-1018`: the state change (he is holding) and the transmission (telling him so) were once one statement, and a lone arrival with the letdown free got "hold at five thousand, right turns, one eight zero outbound one minute..." *and* "cleared radar approach" in one breath. Now the hold is only spoken if he is not about to be cleared. If you hear a hold followed immediately by a clearance, this guard has failed.
- **`_hold_phrase()` changes shape with the aeroplane.** `controller.py:394`. A published hold is only offered when the field has one *and* `equipment.can_hold_at(kit, navaid)` says this airframe can find it. Otherwise it is a racetrack in headings and minutes — "hold at eight thousand, right turns, one eight zero outbound one minute, then three six zero inbound one minute". `kit` comes off the airframe on radar via `note_equipment()` (`controller.py:380`) and `equipment.RECEIVERS`. `kit is None` means nobody told us, and at a beacon field that is treated as able.

### Sequencing, and who may be vectored

`owns_the_approach()` (`controller.py:593`) is the join between the queue and the geometry. The vectoring thread asks it and works only that aircraft.

`may_be_vectored()` (`agent_atc.py:1530`) is the invariant expressed as one question, and it says NO for five separate reasons in order:

1. He is not on the board at all — `ctl._resolve(cs) not in ctl.aircraft` (`agent_atc.py:1569`). Two Jugs transiting feet-wet were vectored onto the Batumi final the whole way to their ingress point before this.
2. He is on a **visual** — `ac.on_visual` (`agent_atc.py:1576`). Reading ranges to a man looking at the runway is chatter.
3. He has not checked in **on this frequency** — `_heard_on` (`agent_atc.py:1591`). Approach used to start working an aeroplane while the pilot was still turning the knob.
4. Single ship, no traffic → YES (`agent_atc.py:1595`).
5. With traffic: **nobody cleared means nobody vectored** (`agent_atc.py:1597`). This is the case that was wrong and produced "we have duplicate controllers again" — two Mustangs holding at five and six thousand both told to turn onto the intercept.

Note `traffic` is passed in from what the *scope* sees (`len(fixes) >= 2`, `agent_atc.py:2782`), not from the board — radar does not forget over a bridge restart and the blind engine does.

### The approach phases: decided from geometry, never remembered

`asr.guide()` (`asr.py:453`) is a pure function of Position + profile. Its answer, `Guidance.phase`, is one of `final`, `map`, `vector`, `missed` (`geometry.py:155-176`). The decision order in `guide()` is the thing to hold in your head when a call surprises you:

1. **Translate to the touchdown point** (`asr.py:490`, `at_touchdown` `asr.py:429`). Radar's reference at Batumi is the runway *centre*; `touchdown_offset_nm=0.559` moves it to the threshold. Every range call and the whole descent profile hang off this. "I was always too high" was this being absent.
2. **Established?** `on_the_course` = within tolerance AND heading within 60° of the course (`asr.py:555-557`). The heading check is not pedantry — a go-around tracking outbound sits on the centreline and was once called established and sent to minimums while flying away.
   ...
   - more than a mile past the threshold → falls out of this branch entirely, deliberately (`asr.py:576-578`), and is a repositioning problem.
3. **On the missed, and not finished it** → **`missed`** (`asr.py:603`). This is the ONE fact geometry cannot decide (see below).
4. **`in_position()`** (`asr.py:254`) → **`vector`** onto the intercept. Two ways to be in position: near enough as an *angle* (`STEER_ON_DEG = 30`) with room to roll out, or `has_room()` (`asr.py:291`).
5. **Just off the departure end, lined up, low** → **`missed`**, the published one (`asr.py:647`). All three conditions are required; without them an aircraft arriving from the north-east gets the missed approach it never flew.
6. Otherwise → **`vector`** on the base leg / downwind (`base_leg_heading`, `asr.py:329`).

`has_room()` (`asr.py:291`) is one line on purpose: `along >= final_intercept_nm + |xtk| + TURN_IN_NM`. At a 45° cut `along` and `xtk` fall together, so `along - xtk` is invariant and an aircraft that has room *keeps* having room. Four earlier versions decided it afresh each sweep with disagreeing conditionals and turned aircraft in, then out, then in, forever.

**The one latch.** `_flying_missed` (`agent_atc.py:1457`) lives in the bridge, not in `asr.py`, and `flying_the_missed()` (`agent_atc.py:1460`) maintains it. Halfway round a 200° missed-approach turn the aeroplane is on nobody's track, so every stateless test flickers and every flicker is a reversal on the radio. Set by geometry recognising the procedure *or* by `Aircraft.approaches` going up (the count, not the phase — `report_missed` re-clears him in the same breath so the phase reads CLEARED again, `agent_atc.py:1476-1486`). Released at `missed_climb_ft` or on leaving the terminal area (`agent_atc.py:1470`).

`phases.py` is not on the live path except through `_PHASE_OF` (`agent_atc.py:309`), which translates the engine's phase names into the official list before writing them to the director's `flights` table. `PHASES` (`phases.py:44`) is deliberately complete while the code is not — `handler=None` phases are declared and unimplemented, and `phases.FLOWN` is just `("approach", "missed")`. `phases.guide()` (`phases.py:151`) dispatches both to `asr.guide`.

### The ASR talkdown and its automatic per-mile calls

`asr_monitor()` (`agent_atc.py:2697`) is a daemon thread. Every `ASR_POLL_SEC = 4.0` seconds it re-reads the scope and, for every radar-**identified** contact (`radar_fixes`, `agent_atc.py:923` — untagged blips are skipped on purpose), does this:

1. `is_on_the_ground()` → say goodbye once, via `ctl.report_down()`, and stop flying him (`agent_atc.py:2798-2846`). The sim's `land` event is preferred over the geometry, which called it 19 seconds early.
2. `may_be_vectored()` → skip if not.
3. `asr.guide()`.
4. `phase == "vector"` → issue a heading, but only if it has moved `VECTOR_CHANGE_DEG = 12` and not more often than `VECTOR_MIN_SEC = 20`. **A change of more than 60° is held for one poll to see if it persists** (`agent_atc.py:2883`) — this does not fix the flip, it stops the radio carrying it.
5. `phase in ("final", "map")` → one call **per whole mile**, keyed on `called[cs] == mile` (`agent_atc.py:2932`).

The mile call text is `asr_call()` (`agent_atc.py:1222`). What goes in it:

- range via `asr.spoken_range` (`asr.py:692`), with the "one mile"/"miles" fix at `agent_atc.py:1233`.
- deviation via `spoken_deviation()` (`agent_atc.py:942`) — a *distance*, not just a side, because "left of course" is an assertion a pilot can and did argue with.
- the correction: **relative** while established (`relative_correction()`, `agent_atc.py:968` — "turn left ten degrees", so gyro drift and magvar cancel), absolute heading otherwise.
- altitude via `altitude_instruction()` (`agent_atc.py:1004`), which uses `Guidance.descend_to_ft` — **the NEXT mile's altitude**, computed by `asr.descend_to()` (`asr.py:103`). The four-mile call carries the three-mile altitude. This is anticipatory on purpose; if you hear the altitude you should *already* be at, `descend_to_ft` came back None.
- speed via `speed_instruction()` (`agent_atc.py:1057`), appended to the altitude clause rather than made its own transmission.

Everything transmits under `radio_lock` and behind `channel_is_free()` (`agent_atc.py:2567`), which returns False if a pilot is transmitting, if the agent is composing an answer (`answering[0]`), or if we are inside a readback window. **A held call is not marked as issued, so it repeats** — log lines `  .. holding the 4 mile call for Falcon 1-1: answering a pilot`.

All of this goes out on ONE frequency: `final_hz`, which is Approach's when `profile.guidance == "talkdown"` and Tower's otherwise (`agent_atc.py:2728-2733`). Getting that wrong produced "two personalities" — conversation on one channel, vectors on another.

### `reconcile()` — what it suppresses and why

`reconcile()` (`agent_atc.py:1391`) is the arbitration point, called once per pilot transmission at `agent_atc.py:3256`. Three authorities have an opinion: the separation engine (owns the queue, cannot see), the vectoring (owns the geometry, cannot remember), the agent (owns the words). They used to be appended side by side, each labelled authoritative, and **the model was left to arbitrate — so it said both.** A pilot established on final at ten miles was told, in one transmission, that he was on final AND to climb to five thousand and hold.
...
- **`hush_a_second_talkdown()`** (`agent_atc.py:1193`). On `final`/`map`, if the agent's reply contains anything matching `_TALKDOWN_WORDS` (`agent_atc.py:1104`) it is replaced with nothing. The cost of not doing this was not merely noise: the agent's chatter held the metronome off the air, and the 6, 5, 4 and 3 mile calls — and their descent instructions — never went out at all. Log: `  .. hushed the agent on final: the engine is flying the talkdown`.
- **The position-report veto** (`agent_atc.py:1726-1734`). A claimed beacon passage is checked against the range of the *identified* track bound to that callsign; more than `OVERHEAD_NM = 4.0` out and the report never reaches the blind engine. Only on a beacon approach (`beacon_flown`) — on an ASR the classifier files every position report as REPORT_BEACON, and without that guard the controller answered "negative, you are not over the beacon" to "on final, runway one three" at two miles. Log: `  !! rejected: claims the beacon, radar shows 8.0 nm`.
- **`seen_on_final()`** (`controller.py:611`, called at `agent_atc.py:1770-1779`). An aircraft radar shows established *owns* the letdown rather than queueing for it. Without it a flight established at ten miles and two thousand feet checked in, was filed as a fresh arrival, and was told to climb to the bottom of the holding stack while the vectoring half was talking it down. Log: `  .. Falcon 1-1 is already on final per radar; not stacking him`.
- **`reads_back_what_we_said()`** (`agent_atc.py:1167`). Not a suppressor but the same class of fix: the engine recomputes continuously, so between issuing 140 and hearing the readback it wants 135, and the agent answered a correct readback with "negative". A readback is judged against `_issued`, what was actually *said to him*.
...
- **The base-leg geometry is built around a 3 nm turn radius.** `tools/asr_sweep.py:277` records the baseline: clean 1296 arrivals / 0 rapid reversals, sloppy 1294 / 35, and at 450 kt the controller argues with itself 181 times. A 5.1 nm turn radius cannot turn inside a 3 nm base leg, so the engine orders a turn, the aeroplane cannot make it, and the engine orders the opposite. That is recorded as known-open, not as a target.
...
- **`asr.py` is the ASR half only.** `safe_alt()` (`asr.py:149`) delegates the *when* of a descent to `atc/descent.py` and floors it with `profile.min_safe_ft(radial, range)`. If an aircraft is told to climb into terrain, that floor and `route.py`'s `msa_sectors` are where to look — an aircraft over the threshold at 600 ft was once told to climb to thirteen thousand on a heading into the Caucasus, which is why the "past the field and low" branch (`asr.py:647`) exists at all.
...
| step | code | note |
|---|---|---|
| sim → cache | `tracks._stream_category` (`tracks.py:193`) | one `mission.StreamUnits` subscription per category (airplane, helicopter, **ground, ship**), upsert on update, `DELETE` on `gone` |
| cache | `tracks` table (`tracks.py:127`) | `name` (PK, the sim unit name), `label`, `type`, `coalition`, `geog`, `alt_ft`, `heading`, `speed_kt`, `player`, `last_seen` |
| cache → lines | `tracks.radar_cached` (`tracks.py:286`) | one PostGIS query; `ST_Distance`/`ST_Azimuth` off Batumi; `ORDER BY nm`; **`last_seen > now() - 15 s`** (`FRESH_SEC`, `tracks.py:120`) |
| lines → string | `dcs.radar_picture` (`dcs.py:164`) | `" | ".join(lines)`, or the literal `no contacts` |

`label` is `u.player_name or u.callsign or u.name` (`tracks.py:163`) — so **a manned contact is labelled by the human's DCS player name, not by his callsign**. That single fact is the reason `radar_fix_by_track` exists (below).
...
Written at `tracks.py:515-518`, with each other ship rendered by `_other_ship` (`tracks.py:440`):
...
Real line (`tests/test_formation_scope.py:50`):
...
Note the separator is an em dash `—` (U+2014). `_FORM_SPAN` and `_FORMATION` accept `--` as well (`identity.py:83`, `identity.py:89`), so a hand-typed test picture with a hyphen dash still parses, but the renderer only ever emits `—`.

### Every field, and what breaks if it is wrong

| field | source | who consumes it |
|---|---|---|
| `label` (leading name) | `player_name or callsign or name`, deduped by `_unique_labels` (`tracks.py:416`) | `identity.units_on` → `unit_for_radio` (the SRS-name→unit chain); `_FIX_BY_TRACK` |
| `[callsign]` tag | `contacts` table via `bindings_for` (`director/tools/identify.py:101`), TTL **2 h** (`identify.py:BINDING_TTL_SEC`) | `_FIX` / `radar_fix`, `radar_fixes`, `_RANGE` / `radar_range_for`, `_TYPE` / `aircraft_type_on_scope`. **Only tagged contacts are visible to these.** |
| `type` | sim's `u.type` | `aircraft_type_on_scope` → `equipment.receivers` → whether he can be sent to a beacon or must be vectored |
| `manned` | non-empty `tracks.player` (`tracks.py:485`) | `identity.Unit.manned` → `Registry.by_elimination` (`identity.py:328`). **No `manned` markers anywhere = no guest can ever be identified by elimination.** |
| `on the ground` | `events.on_the_ground()` — the sim's land/takeoff **events**, not an altitude guess (`tracks.py:472`, `director/tools/events.py:198`) | `is_on_the_ground` (`agent_atc.py:1964`) → silences approach guidance, drives the Tower handoff. Absent means "airborne **or** nothing has told us"; the caller keeps an alt/speed fallback |
...
| `heading` | `u.orientation.heading` — **the sim's x/z GRID frame, not true** | converted at the parse boundary by `true_heading` (`agent_atc.py:764`); Batumi's `grid_convergence_deg = 5.74` (`core/route.py:1184`). Mixing frames drew every centreline six degrees off the runway |
...
Bridge side, all tolerant on purpose — a picture that throws takes every aeroplane's identity down with it (`identity.py:169`).

| parser | file:line | what it wants |
|---|---|---|
| `identity._SCOPE_LINE` | `identity.py:73` | `NAME [TAG]? (TYPE)` followed by **a colon OR the start of `IN FORMATION`**. The lookahead is the fix for the scar below |
| `identity._FORMATION` / `_OTHER_SHIP` / `_split_ships` | `identity.py:83,84,109` | the ship list between `IN FORMATION with` and the dash. `_split_ships` splits on commas **at paren depth zero** — a plain split produced an aeroplane called `manned)` |
| `identity.flatten_formation` | `identity.py:93` | deletes the whole `IN FORMATION with … — N ships, lead ` span and substitutes `: `, turning a formation line into an ordinary lead line |
| `agent_atc._FIX` | `agent_atc.py:724` | `[TAG] … N nm … on the NNN radial … N,NNN ft … heading NNN? … N knots?` — **requires the bracket tag** |
| `agent_atc._FIX_BY_TRACK` | `agent_atc.py:785` | the same line found by the **unit name**, tag optional; requires the colon (hence the flatten) |
| `agent_atc._TYPE` | `agent_atc.py:733` | `[TAG] (TYPE…)`, then `.split(",")[0]` to drop `manned` |
| `agent_atc._RANGE` | `agent_atc.py:717` | `[TAG] … N nm` — the cheap one, used only by `radar_range_for` |
| `agent_atc._SHIPS` | `agent_atc.py:680` | `(\d+) ships` — `count_contacts` sums these so a four-ship counts as four, not one, and the separation engine actually switches on |

Every one of these bounds itself with `[^|]` so a match cannot bleed across the `" | "` separator into the next contact.

`radar_fix`, `radar_fixes`, `radar_fix_by_track` and `aircraft_type_on_scope` all call `identity.flatten_formation(scope)` first (`agent_atc.py:744, 817, 888, 933`). The reason is at `identity.py:97`: on a formation line the **first** `N nm` after the name is the wingman's 0.3 nm offset, so every "first nm wins" regex would have put a flight three hundred yards off the runway. Flattening was chosen over teaching four patterns about formations — one place to be right instead of four to keep in step.
...
- `radar_fix(scope, cs)` (`agent_atc.py:853`) searches for a **bracketed callsign tag**. A tag is a label: it can be stale, mis-heard, or simply not applied yet. It matches `.canonical` **first** across the whole picture, and only falls back to `.is_flight` + matching flight number — because matching on `.flight` gave Falcon 1-1 and Falcon 1-2 each other's geometry live, one being told "one mile from the runway" at thirty-six miles (`agent_atc.py:864-887`).
- `radar_fix_by_track(scope, track)` (`agent_atc.py:792`) searches for the **sim unit name** that `identity.Registry.resolve` already bound this radio to. Never spoken, cannot be stale, and it finds **untagged** contacts, which `radar_fix` structurally cannot.

The live loop tries track first, name second: `_fix = radar_fix_by_track(...) or radar_fix(...)` (`agent_atc.py:3167`, `3440`), and the radar-contact flag handed to the separation engine does the same (`agent_atc.py:1758`). If the pilot is told he is not radar identified while a clean line for him is sitting in the `RADAR:` block, that is this seam — check whether `(identity: …)` printed and whether the tag on his line matches the callsign the engine is using.

### Formation clustering

`_clusters` (`tracks.py:326`) groups contacts before rendering. Thresholds at `tracks.py:321`:
...
Membership is chained against **any** member of a group, not against the leader — a four-ship in trail is longer than the pairwise threshold end to end, and comparing everyone to the lead drops the tail (`tracks.py:366`).

Why it exists: four ships a mile apart at the same altitude on the same heading are four indistinguishable blips, and the controller's own rule is that an ambiguous match must not be identified — so **the more aircraft in a formation, the less he could see it**, and a four-ship could never be radar identified at all (`tracks.py:329`).

`FORM_FT = 500` is deliberately well under the holding stack's 1,000 ft step. Raise it past 1,000 and the detector eats the stack: four aircraft you have just correctly separated over the beacon merge into one contact, and the controller is told that four aeroplanes are one (`tracks.py:317`).

`in_formation(label)` (`tracks.py:377`) runs the same clusterer for a different question — asked before binding a *flight* name to a track, because "Pony 1" is one aeroplane when alone and a formation when not. Its query returns **the same columns in the same order** as the picture query and casts the Postgres `Decimal`s to `float`, because the two had drifted and handed `_clusters` rows of different widths (`tracks.py:398`). Relatedly, `near()` indexes the row **by position** (`a[3], a[4], a[6], a[7]`) rather than unpacking, because the row has grown twice and each time the unpack became a `ValueError` that only fires **when two contacts are compared** — a single ship never reaches it, so every one-aeroplane test passed and the first two aircraft on the scope would have lost the whole radar picture (`tracks.py:343`).

### FORM_NM is 2.0. JOIN_NM is 1.0. Do not conflate them.

`FORM_NM = 2.0` (`tracks.py:321`) is the *renderer's* opinion about what looks like a formation. `JOIN_NM = 1.0` (`src/marshall/atc/flights.py:64`) is the *rule* for admitting a man to a flight, enforced at `flights.py:304`.

So **"radar shows them as a formation" is not evidence they are within a mile** — it is evidence they are within two. Treating it as proof would silently double the join radius. That is exactly why `_other_ship` prints the offset and the rule keeps measuring (`tracks.py:507-512`, `tests/test_formation_scope.py:25`).

`miles_between` (`agent_atc.py:519`) is what does the measuring, on top of `_scope_geometry` (`agent_atc.py:563`), which returns two dicts because a formation genuinely has two kinds of member: `pos` (absolute, from range+radial) and `offset` (`wingman → (lead, gap)`). The cases:
...
Its only consumer is the beacon-overhead sanity check (`agent_atc.py:1729`), which rejects a "over the beacon" report when radar shows him more than `OVERHEAD_NM = 4.0` out. Under-reporting the range means the check **fails to fire**: a tagged flight lead 13.5 nm from the beacon claiming station passage sails through, and the blind engine breaks the formation up early — the exact live failure the surrounding comment describes ("a flight called 'over the beacon' at eight miles… the two brains then disagreed about where four aeroplanes were"). It only bites on beacon-flown (non-vectored) profiles. Every other scope regex was taught about formations in `67080bf`; this one was not.

**The live-gRPC fallback renders a different, poorer line.** When the PostGIS cache cannot be read, `radar_picture` falls back to `radar_live` (`dcs.py:148`), which emits:
...
**Deploy skew is a real state.** The bridge and the director restart independently, so an older picture without wingman offsets is something the parsers must survive; that is why `_OTHER_SHIP`'s parenthetical is optional and why the recorded `OLD` string is kept verbatim in the test file (`tests/test_formation_scope.py:41`).
...
The bridge prints the picture it actually used on every transmission (`agent_atc.py:3163`):
...
## route.py: one source of truth, four consumers
...
(`route.py:1-10`) is the one to hold onto when something disagrees on the radio:
...
airspeed indicator is (`route.py:8-9`). `MPH_PER_KT = 1.15078`
(`route.py:16`) is the conversion, and forgetting it is a recurring 15% error the
file warns about twice (`route.py:806-808`, `route.py:1341-1343`).

### What the module defines

| Thing | Where | Read by |
|---|---|---|
| `NM`, `MPH_PER_KT`, `MAGVAR` (6.0°E) | `route.py:15-21` | everything |
| `CRUISE_TAS_MPH`, `CRUISE_ALT_FT` | `route.py:24-25` | mission builder, nav log |
| `WIND_FROM_DEG` / `WIND_MPH` (180 at 5 kt) | `route.py:40-41` | mission weather, nav log, plate, landing clearance |
| `QNH_MMHG`, `qfe_inhg()`, `altimeter_spoken()` | `route.py:49-69` | mission weather, the plate's spoken setting, comms card |
| `FLIGHT_CALLSIGN`, `SQUADRON_CALLSIGNS`, `FLIGHT_SIZE` | `route.py:75-90` | mission group names **and the Whisper vocabulary prime** |
| `Fix` (name, ident, x, z, freq, sector, `navaid`) | `route.py:93-110` | fix table pushed to the director, plate, chart |
| Named fixes: `KOBULETI`, `INITIAL`, `BATUMI`, `KUTAISI`, `TARGET_AREA`, `AIR_START`, `FEET_WET`, `INGRESS`, `HOMEBOUND` | `route.py:117-200` | as above |
| `SORTIE`, `SORTIE_LEGS`, `SORTIE_ALT_FT`, `steerpoint()`, `sortie_points()` | `route.py:191-264` | nav log, route map, mission brief, the agent's `_sortie()` block |
| `DEFENDED` (flak fields + reach) | `route.py:171-175` | route map rings, the agent's `_threats()` block |
| `MSA_SECTORS` (2 published sectors) / `MVA_CELLS` (48 surveyed cells) | `route.py:294-360` | vectoring floor, plate |
| `Station` (name, freq, role, `also`, **voice**) and `CENTER/APPROACH/TOWER/OVERLORD` | `route.py:406-466` | SRS channels the bridge monitors, per-controller Polly voice, radio presets, comms card |
| `Field_` / `BATUMI_FIELD` | `route.py:469-493` | elevation, MSA/MVA tables the profile copies |
| `AtcCapability` | `route.py:496-512` | plate generation + whether the bridge feeds radar at all |
| `ApproachProfile` and the two instances | `route.py:519-1230` | *everything* |
| `bearing_distance`, `wind_triangle`, `magnetic`, `solve_route` | `route.py:1235-1346` | nav log, brief, route map |
| `profile_to_dict` / `profile_from_dict` | `route.py:1270-1307` | the director's `approaches` table |

`route.py` is also runnable on its own — `uv run python -m marshall.core.route`
prints the wind-corrected leg table (`route.py:1349-1361`).

### There are TWO profiles, and only one of them is flying

This is the first thing to check when something reads wrong.

| | `BATUMI_APPROACH` (`route.py:1101`) | `BATUMI_ASR` (`route.py:1146`) |
|---|---|---|
| `kind` | `"ndb"` | `"asr"` |
| `vectored` (`route.py:744`) | False | **True** |
...
`load_and_push_plate(R.BATUMI_ASR)` at `agent_atc.py:2491`. But
`BATUMI_APPROACH` is still referenced in two places that matter:

- **`mission/build.py:404`** — `P = R.BATUMI_APPROACH` is what the *weather* is
  built from. It reads `P.ceiling_ft` (400) to set the overcast base.
- **`kneeboard/plate.py:28`** — the "NDB 13" tab is still drawn from the letdown
  profile, and `site.pages()` still lists it unconditionally (`site.py:70-72`).
...
Use `--ceiling`/`--tops` (`build.py:911-914`) rather than `--hard` until those
two lines read the same profile.

Related: `cloud_thickness_ft=3000` on the profile (`route.py:558`) is what
`tops_ft` and therefore the *stack base* are computed from, but `build.py:423`
...
|---|---|---|
| `mda_ft` (`route.py:901`) | `max(field_elev + min_hat_ft, ceiling - breakout)` | 732 |
| `stack_ft` (`route.py:1059`) | `hold_base_ft`, raised to clear `tops_ft + vmc_margin_ft` when vectored, stepped to `hold_top_ft` | 5000…10000 |
| `hold_in_clear_air` (`route.py:1040`) | `stack_ft[0] > tops_ft` | True |
| `final_crs_true` (`route.py:888`) | measured value if present, else `final_crs + magvar_deg` | **131.0** (measured) |
| `speed_kt` / `final_speed_kt` (`route.py:819-828`) | mph ÷ 1.15078 | 208 / 174 kt |
| `speed_kt_at(along, established)` (`route.py:830`) | final speed inside `final_intercept_nm`, pattern speed outside | — |
| `hands_to_tower_nm` (`route.py:1012`) | `map_nm` on a talkdown, `final_intercept_nm` otherwise | **0.6 nm** |
| `min_safe_ft(brg, rng)` (`route.py:690`) | MVA cells → MSA sectors → platform, never below platform | — |
| `inbound_descent_nm` / `final_approach_sec` (`route.py:917-931`) | platform→MDA at `descent_fpm` | — |
...
(`route.py:1155-1181`). Radials come from `ST_Azimuth` on lat/lon (true); a
heading arrives from the sim in the grid frame and is converted by
`agent_atc.py:780` using `grid_convergence_deg`. If the centreline is ever six
...
the same controller was mid-talkdown (`route.py:1012-1033`).

`touchdown_offset_nm = 0.559` (`route.py:1182-1183`, used at `asr.py:442`) is
...
`AtcCapability` (`route.py:496-512`) is four fields and the defaults describe a
...
- **`radar`** is read once, at `agent_atc.py:2492` (`radar_on = profile.atc.radar`),
  and it decides whether the bridge feeds the agent a radar picture at all. It is
  also read at `agent_atc.py:1791`, where a radar-equipped controller has the
...
- **`dme`** is *the aeroplane's* equipment, not the controller's, and it is
  currently only surfaced in the plate text (`briefing.py:288-290`). It is why
...
`kind="ndb"` (`route.py:1122`). Flip `radar` off there and you get the fully
blind classic. `BATUMI_ASR` keeps the ww2 era word but sets
`separation="radar"` (`route.py:1229`) — the handicaps that defined the letdown
...
`controller.py` is field-agnostic by design (`controller.py:1-33`): "Hand it a
...
- `stack_ft` / `bottom_ft` / `top_ft` for the holding ladder
  (`controller.py:567`, `942`, `1063`)
- `station(enroute=…, banished=…)` for who to hand off to and on what frequency
  (`controller.py:472`, `665-667`) — see `route.py:933-964`, and note it resolves
...
- `beacon.name` and `beacon.navaid` for the hold phrasing (`controller.py:429-432`)
- `outer_hold` for banishment after two misses (`controller.py:839-842`)
...
(`agent_atc.py:2494-2520`). The voice lives on the `Station`
(`route.py:429-434`) precisely so it cannot drift from the identity it belongs
...
third of the agent's system prompt. `plate()` (`briefing.py:261`) branches on
...
- `_formation()` (line 72) — deliberately says nothing about *which* flight is
  ...
  greeting him already knowing where he was going (`briefing.py:74-81`)

The push path, `agent_atc.load_and_push_plate` (`agent_atc.py:374-414`), runs
...
2. `PUT /flightplans/362nd-batumi-asr`, then `GET /flightplan/active`, and
   **rebuilds the profile from the DB row** via `profile_from_dict`
   (`agent_atc.py:390-398`). This is why the log line reads
   `loaded flight plan '…' -> approach '…'`
3. `push_fixes` (`agent_atc.py:2405`) — projects every `Fix` in `route.py`
...
`director/app.py:60-63` assembles the system prompt as
...
(`director/strands_pg/prompts.py:89-132`) only overwrites a row when the *file
...
(`build.py:1-3`). What it pulls:

- Group names from `SQUADRON_CALLSIGNS` (`build.py:88-90`) and the flight group
  name via `C.parse(R.FLIGHT_CALLSIGN).flight` (`build.py:197`)
- Spawn points from `R.AIR_START` / `R.INITIAL` (`build.py:198`, `250`, `359`,
  `490`, `537`), altitudes from `R.CRUISE_ALT_FT`, speeds from
  `R.CRUISE_TAS_MPH` — per airframe, because "the Mustang's number spawned a
  Thunderbolt on the edge of the stall" (`build.py:44-47`)
...
- Weather: wind direction inverted to DCS's "blowing to" convention
  (`build.py:444`), speed converted mph→m/s, and `m.weather.qnh =
  int(round(R.QNH_MMHG))` (`build.py:448`) so the setting the controller says
  and the setting the sim is running are one number
- The radio card: `channels_for(profile)` (`build.py:741`) derives (button,
...
- `set_channels` (`build.py:773`) writes presets only to a radio box that can
  actually *tune* the frequency, via `_band_of` on the airframe's own pydcs
  defaults; `write_presets` (`build.py:835`) additionally injects the WW2
...
- `_brief_text` (`build.py:672`) renders the same legs and the same four
  channels into the DCS slot-screen brief

**Beacons no longer transmit.** `build.py:604-620` emits only comment lines into
...
A consequence nobody has cleaned up: `mission/validate.py:60-66` still asserts
...
`site.pages(profile)` (`site.py:43-75`) derives the tab list from the profile, so
pointing `route.py` at another field gives that field's kneeboard with no edit
here. Tabs and their sources:

| Tab | Module | Reads |
|---|---|---|
| BRIEF | `brief.py:41` | stations by role, `solve_route(SORTIE_LEGS)`, QFE via `qfe_inhg`, wind |
| ROUTE | `routemap.py:38` | `SORTIE`, `DEFENDED` rings, `TARGET_AREA` |
| COMMS | `comms.py:55` | the station list in the order the buttons get pressed, plus the spoken QFE |
| NAV LOG | `navlog.py:102` | `SORTIE_LEGS`, `leg_altitude`, `ias_mph`, `STATIONS`, `BATUMI_FIELD.msa_sectors` |
| `ASR 13` | `asr_plate.py:32` | the profile — course, MDA, frequencies, and *what the controller's calls will sound like* |
| `PLATE 13` | `aip_plate.py:46` | only appears if `plate_png` names a scan we actually hold (`has_plate`, line 42) |
| NDB 13 | `plate.py` | **`BATUMI_APPROACH`** — the old letdown chart, still listed |
| E6B | `e6b.py:71` | `MAGVAR`, `CRUISE_TAS_MPH`, wind, as the calculator's starting state |

`asr_plate.py:1-16` is explicit that it is not a rework of `plate.py`: an ASR has
...
`kneeboard/serve.py:71` watches `marshall.kneeboard` **and
`marshall.core.route`** for reload, so editing `route.py` re-renders the pages
without a restart.

### equipment.py: what the aeroplane can receive, and how slow you may ask it to fly
...
**Receivers are a SET, not a ladder** (`equipment.py:8-29`). The headline case:
...
`RECEIVERS` (`equipment.py:44-72`) maps the type string the sim reports on radar
...
ever flyable. An unlisted type gets `ASSUMED_MODERN` (`equipment.py:78`),
deliberately *without* ADF.

`RECEIVER_FOR` (`equipment.py:185-191`) exists because the two vocabularies do
...
The path into the controller: `agent_atc.py:1765-1766` calls
`_eq.receivers(type)` off the radar contact and hands it to
`ctl.note_equipment` (`controller.py:380`), which stores it on
`Aircraft.kit` (`controller.py:84-98` — note the comment about why that field is
...
`Controller._hold_phrase` (`controller.py:393-450`) then decides between two
...
**Speed targets are per airframe** (`equipment.py:81-147`). `MIN_VECTOR_KT` is
...
aeroplane wins. It is called from `agent_atc.py:1091`, and that function returns
`""` outright when the phase is `final`/`map`/`missed` (`agent_atc.py:1080-1087`)
...
1. **Two profiles.** `mission/build.py:404` and `kneeboard/plate.py:28` still
...
3. **`mission/validate.py` still checks for beacons** that were deliberately
   removed.
4. **`profile_from_dict` silently drops unknown keys** (`route.py:1306-1307`).
...
5. **`mva_for` answers for where the aircraft is *now***, not for the ground
   under the vector it is about to fly (`route.py:386-388`). At Batumi every
...
| created | `tools/approaches.py:39` (name, callsign, approach, weather, active), extended by `migrations/009` (origin, destination, route, cruise_ft, task) and `010` (label) | `migrations/009_flight_plans_are_templates.sql` |
...
| read by | `clearance.filed()` (`clearance.py:61`), `GET /plans` | `clearance.assigned(flight_id)` (`clearance.py:80`), the `flight_with_plan` view |

The unique index is the design, not housekeeping: a second assignment to the same flight is an **amendment** and replaces the row (`ON CONFLICT (flight_id) DO UPDATE`, `clearance.py:105-111`). Two live plans for one aeroplane is precisely the ambiguity the table exists to remove.
...
**One table, two unrelated jobs — this is the confusing part.** The same `flight_plans` rows also carry `active`, and `active_flight_plan()` (`approaches.py:98`) joins the one active row to its `approaches` record to build the **plate**. So `GET /flightplans` (`app.py:232`, columns name/callsign/approach/active) and `GET /plans` (`app.py:299`, columns name/label/origin/destination/route/cruise_ft/task/approach) return the *same rows through different windows*. When you are chasing "the flight plan", establish first whether you mean the plate selector or a clearance template.

### A plan on file belongs to nobody

`_TEMPLATE_COLS` (`clearance.py:57`) deliberately **does not include `callsign`**, so the column cannot influence matching even by accident:
...
A plan becomes his at the moment a clearance is issued, by being copied into `assigned_plans` against his `flight_id`, and not one instant earlier (`clearance.py:45-56`). There used to be a branch in `plans.pick` that handed over the one plan carrying his callsign as the civil "as filed" case; it is gone, and the long comment at `plans.py:141-156` says why — a template callsign is a word a mission builder typed, attached to no person and no flight, the same kind of name as the "Falcon" that cost a sortie. Guarded by `tests/test_plans.py:289` (`TestAPlanOnFileBelongsToNobody`), including `test_the_column_is_not_even_read`.

Any pilot may take any plan. `filed()` is unfiltered on purpose (`clearance.py:61-72`) — flying somebody else's filed sortie is normal here and impossible in the civil world.

`assign()` writes **twice**, in one call (`clearance.py:98-124`): the copy into `assigned_plans`, and then `flights.agree(...)` stamping `flight_plan`, `destination`, `route`, `cruise_ft` on the flight's own row, because that is what the state view, the plate and "where is he going" all read. `acked_at` is set to NULL by the amendment branch on purpose — he read back the clearance he was given, not the one that replaced it.

### Matching what he said to a plan

`plans.score()` (`plans.py:77`) runs over four fields, weighted by how much each is worth here:
...
`plans.pick()` (`plans.py:113`) then returns one of exactly three shapes, and telling them apart is the whole job:
...
The `best == 0` branch (`plans.py:126-159`) is the subtle one. If he named *something* and nothing scored, the answer is `none` — offering a menu of three plans that all go somewhere else answers a question he did not ask. But "named something" is only decidable when his callsign is known, because that is what lets his own name be subtracted from what he said (`plans.py:136`); without it, "Hoover" is a word that matches no plan and every request would read as a request for somewhere nobody filed for. So an unknown caller gets asked, not refused. When he named nothing at all: one plan on file → that one ("the only one on file"); more than one → the whole list.

A tie at the top is `ambiguous` (`plans.py:161-163`). **Nothing here picks the first, the newest or the best-scoring.** A resolver that always picks looks perfect in a demo because every request produces a clearance — including the ambiguous ones, where the clearance is for somebody else's sortie.

`ask_which()` (`plans.py:167`) is what the controller says: `"I have CAS over Tsutsnvati, filed as Samovar; or CAS over Tsutsnvati, beacon letdown on return, filed as Kettle. Say which."` Note "**filed as**", not a bare name — handed the label alone the model called it a *callsign*, and a pilot hearing "callsign Kettle" has been told there is an aeroplane out there called Kettle.
...
The `--live` run is how you catch the sweep's inline copy of the five plans (`tools/plan_sweep.py:42`) drifting from what migration 012 actually put in the database.

### The CRAFT clearance, and why the words come back finished

`plans.clearance()` (`plans.py:231`) returns the finished sentence in the order a pilot writes it down:
...
The tool returns words rather than facts for the same reason the deterministic controller's altitudes are voiced rather than paraphrased (`clearance.py:1-19`, `app.py:107-112`): a squawk, a cruise altitude and a departure frequency are facts about what was *filed*, and a controller who improvises them has cleared somebody to an altitude nobody wrote down. `rules.md:137-151` is the other half of that contract — read it verbatim, read all of it, the departure frequency is the element most often lost.

Details worth knowing when something sounds off:

- **"As filed" is load-bearing** and appears only when the route really is the filed one; an amendment reads the route out in full (`plans.py:246-251`). The words "as filed" beside an amended route is a checkable defect.
- **The squawk is deterministic on `flight_id`** (`plans.py:194-214`), so asking twice gives the same code and a pilot reading back what he wrote is not wrong through no fault of his own. It is octal, and it skips `RESERVED_SQUAWKS` (7500/7600/7700/7000/1200/0000/7777). The seed is hashed before conversion because straight modulo gave flight 1 the code 0001. A `clear_mission` between sorties gives the same aeroplane a new `flights.id` and therefore a new code.
- **Numbers are spelled out** (`_spell`, `_spell_alt`, `plans.py:217-228`) because the string goes straight to Polly.
- **The "expect cruise one zero minutes after departure" clause is unreachable today.** `clearance()` emits it only when `cruise != initial_ft` (`plans.py:255`), and `request_clearance` always passes `initial_ft=plan["cruise_ft"]` (`clearance.py:319`). If you are wondering why you have never heard it, that is why — not a Polly or model problem.
- **The departure frequency is dropped silently when it cannot be found.** `departure_freq()` (`clearance.py:153`) reads the *published stations* off the active approach profile: a station whose `role` or `also` contains `departure`, else the first `approach` station, else `0.0`. `clearance()` then omits the element entirely (`plans.py:258`). So if the plate/approach was never pushed, `_stations()` is `[]`, and the clearance goes out complete-sounding and missing the one element `rules.md` calls most often lost. The dry run found this going missing twice.

### Route fixes resolve through the `fixes` table

Coordinates are **not** stored on a plan. `plans.route_fixes(plan, fixes)` (`plans.py:267`) splits the comma-separated route, lowercases each name and looks it up in the fix table, returning `(resolved, missing)`. The fix table is pushed by the bridge at startup from `route.py` through the **sim's own** `coord.LOtoLL` (`agent_atc.py:2405-2478`, stored by `tracks.set_fixes`, `tracks.py:64`), which is why a plan and a chart cannot disagree about where INGRESS is. Every fix `route.py` publishes goes up, not only tonight's legs, because a filed plan may route via KOBULETI which no sortie touches.

**`missing` is refused at delivery, not dropped** (`clearance.py:308-314`):
...
This is correct behaviour with an ugly failure mode. If the fix push failed at bridge start, `_known_fixes()` returns `{}` (`clearance.py:376-384`) and **every** plan has every fix missing, so **nobody gets a clearance all night**. The two lines that tell you:

```
  !! fix push failed, controller has the field only: ...     # bridge log, agent_atc.py:405
no fixes pushed; route resolution will report everything missing   # director log
```

Confirm with `curl -s localhost:8000/fixes`. The fix table is persisted in Postgres precisely so a director restart does not silently strand it (`tracks.py:70-75`), but a bridge that started with the sim down never pushed in the first place. A route name that is merely misspelled in a migration produces the same refusal for one plan only — migrations 011 and 012 both warn about this.

### The read-back

`clearance_read_back(callsign, correct)` (`clearance.py:331`) stamps `assigned_plans.acked_at` and `flights.clearance_ack` with a timestamp, not a flag — *when* he agreed is the useful part (`ack`, `clearance.py:130-142`). Filed, given and **agreed** are three different states and the gap between the last two is a controller's business.
...
2. **Mission-name mismatch.** `clearance_tools()` is constructed with no argument at `app.py:112`, so `mission` is `"default"` for every clearance lookup. The bridge writes flight rows under `MISSION = os.environ.get("MARSHALL_MISSION", "default")` (`agent_atc.py:251`). Set `MARSHALL_MISSION` to anything and **every** pilot on the frequency hears the not-on-the-board refusal while the board looks perfectly healthy in `GET /flights?mission=<yours>`. Check that first if the symptom is universal rather than per-pilot.
...
`/plans/resolve` (`app.py:306`) is pure judgement with no side effects — it is the dry run for the radio, and it separates "the matcher chose wrong" from "he was never on the board" in one call.

### The bridge's two involvements

1. **It rewrites a template's callsign at every start.** `load_and_push_plate` PUTs `/flightplans/362nd-batumi-asr` with `callsign=R.FLIGHT_CALLSIGN` (`"Pony 1-1"`, `route.py:75`) and `active=True` (`agent_atc.py:387`), and `upsert_flight_plan` does `callsign=EXCLUDED.callsign` (`approaches.py:83`). Migration 012 explicitly nulled that column on all five rows; the bridge puts it back on Samovar every boot. It no longer affects plan matching (dropped at `_TEMPLATE_COLS`), but it is the entire content of the bridge's `filed_plans()` identity rung (`agent_atc.py:2342`), which reads `/flightplans` looking for callsigns typed before the sortie. Practical consequence: that rung knows one name, `Pony 1-1`, and only after a bridge restart. A visiting pilot who wants a clean first approach needs a plan row bearing his callsign — which today nothing but this bootstrap writes.
...
|---|---|---|
| Bridge stdout | `/tmp/marshall-bridge-live.log` (`tools/bridge.py:35`, override with `MARSHALL_BRIDGE_LOG`) | What the bridge *decided*: identity, suppressions, which thread transmitted |
| Flight recorder | `build/logs/flight-<session>.jsonl` (`agent_atc.record`, `agent_atc.py:179`) | Machine-readable, one object per transmission, with the geometry attached |
| Director | `cd /opt/marshall/director && docker compose logs -f` | Model errors, `session ... is still answering the previous call` (`director/app.py:171`), tool failures |
...
|---|---|---|
| `PILOT [name]: … [RADAR: …]` | `agent_atc.py:3164` | One transcription arrived. `name` is the *resolved* identity, not what he said |
| `(identity: …)` | `agent_atc.py:3152` | The identity ladder did **not** close on radar. Printed only in that case — see below |
| `CONTROLLER: …` | `agent_atc.py:3472` | The deterministic engine produced a next step |
| `SEPARATION: …` | `agent_atc.py:3474` | The holding stack, shown only with ≥2 aircraft unresolved |
| `ATC[pilot/sonnet] (4.3s): …` | `agent_atc.py:2641` | The agent answered a pilot. The tier and the latency are in the line |
| `ATC[hook/sonnet]` | same | Agent woke on its own timer, not on a transmission |
| `ATC[simple] (0.0s)` | `agent_atc.py:3212` | Canned reply, no model call (`simple_response`, `:89`) |
| `ATC[who] …  (out of the blue, no callsign)` | `agent_atc.py:3396` | The challenge. The agent was never asked |
| `ATC[vec]` / `ATC[asr]` / `ATC[down]` | `:2910` / `:2935` / `:2851` | The **asr_monitor** thread, deterministic, no model involved |
| `ENG[tx]` | `:2980`, `:3368` | Engineering radio, Polly voice "Amy" |
| `HOOK fired (+60s) on 139.000: …` | `:2688` | Scheduler paid back a promise |
...
|---|---|---|
| **ATC said nothing at all** — the pilot transmitted and the frequency stayed dead | (a) The bridge is not running, or a stale one holds the lock; (b) STT returned empty; (c) the director dropped the call as busy; (d) the agent replied without a `RADIO:` marker and was silenced | `uv run python tools/bridge.py status`. Then `grep PILOT /tmp/marshall-bridge-live.log \| tail` — no `PILOT` line means STT or SRS, not the brain. A `PILOT` line with no following `ATC[...]` means the agent: check `docker compose logs` for `still answering the previous call` (`app.py:171` returns an empty response rather than queueing), or an `ATC[pilot/sonnet] (…): (no call)` line, which is `for_voice(agent=True)` at `:132` discarding a reply that had no `RADIO:` marker — the model wrote reasoning instead of a transmission |
| …or the pilot was on **engineering** | Everything he says goes to engineering until he releases the line (`:3335`). Vectors and goodbyes are explicitly held: `.. holding a vector for X: he is on the engineering line` | `grep ENGINEERING /tmp/marshall-bridge-live.log`. Release is by saying a controller's name in the *opening six words* (`_ADDRESSING`, `:2603`) or `_ENG_DONE` |
| …or **two bridges** are up | Both hear the pilot, both answer, and each hears the other. `claim_the_frequency` (`:1921`) is an advisory `flock` on `build/bridge.lock` and should make this impossible now — but a bridge started before the lock existed, or on a second host, still can | `!! another bridge already holds the frequency (pid N)` on start; `(ignoring Marshall -- that is one of ours, not a pilot)` at `:3009` in a running log is the smoking gun |
| **"Station calling, say your callsign"** | `challenge_for` (`:1513`), fired at `:3390`. It means: identity is unknown (`known` truthy but he named nobody), he is *not* inside the 90 s conversation window (`CONVERSATION_SEC`, `:1505`), and no flight verdict was pending | `grep "ATC\[who\]"` in the log. Then look at the preceding `(identity: …)` line for *why* the ladder failed — `"'Pony 1-1' matches no track and no filed plan"` vs `"nobody named, and the radio is unknown"` are different bugs |
| …challenged even though he *did* say who he is | `said_who` (`:828`) matches only against **closed sets** — existing flight names and the handle this radio resolved to. If the radio never resolved to a track, his handle isn't in the set. Also: `extract` wants the numbered shape, so a bare handle used to be missed | `tools/identity_watch.py` — the `claim` column shows what Whisper gave the extractor |
| **Wrong callsign on the air** | Almost always the *label*, not the identity. `identity.Registry._label` (`identity.py:357`) takes what he calls himself, voted across the sortie by `transmitter_callsign` (`:605`). A wrong label is rude; a wrong `track` is dangerous, and they are decided separately on purpose | `tools/whos_who.py` — the RADIO panel prints `heard from` beside `answered to` and marks `<-- MISMATCH`. If the track is right and the name is wrong, it is the vote |
| …"Falcon one **TO** one" | Polly reading a hyphen. Canonical `Falcon 1-1` reached the voice. `for_voice` now rewrites it at `:172`, which is the one place every transmission passes | `grep -E "[A-Z][a-z]+ [0-9]-[0-9]" build/logs/flight-*.jsonl` on the `text` field of `atc/*` records |
| …called by his **flight's** name when he is a single | `known = _flights.speaking_as(_who)` at `:3145` — he is still on the roster as a member. Or `transmitter_callsign` still holds the flight designator; it clears the vote the moment he names himself more specifically (`:650`) | `.. <name>: created/joined/left` lines, and the `flight/*` records in the recorder |
| **Wrong or absent altitude / heading** | Three authorities can speak and only one may. `reconcile` (`:1391`) picks from the *geometry*: missed → the missed approach wins; established on final → the talkdown wins and any holding clearance is dropped; otherwise the separation engine wins and the vector is dropped. Anything dropped prints | `grep "  \.\. " log` for `holding clearance suppressed: radar shows him established`, `vector suppressed: he has been told to hold`, `holding/vector suppressed: he is flying the missed approach` |
| …the **descent call never came** on final | `hush_a_second_talkdown` (`:1193`) is the fix, and the failure it fixes is subtle: the agent's own chatter holds the channel, the metronome's 6/5/4/3-mile calls are deferred by `channel_is_free`, and the aeroplane is into the next mile by the time it clears | `.. hushed the agent on final: the engine is flying the talkdown` (`:2637`), and `.. holding the N mile call for X: <why>` (`:2928`). Repeated "holding" lines with no `ATC[asr]` between them is the bug recurring |
| …an altitude the plate does not contain | The agent invented it. `director/prompts/rules.md` + the generated `plate` prompt pin the assignable levels. Check the plate actually pushed: `pushed plate for <controller> to the director` at startup (`:411`); `!! could not push plate` means the agent is running on whatever was last seeded | `curl -s localhost:8000/prompts` (or the `/approaches` and `/flightplan/active` endpoints) |
| **Two aircraft got each other's position** | The identity ladder bound a radio to the wrong track — or, more often, to *no* track, and everything downstream used the callsign as the key. `radar_fix_by_track` is preferred over `radar_fix` on every path for exactly this (`:3143`, `:3437`) | `tools/whos_who.py`: TRACKS shows every unit and what it is `tied to`; BOARD shows what the engine believes exists; a `!! on the board, not on radar:` line is a ghost made of words |
| …and it started after a slot change | A callsign is a position, not a person (#38). `Registry.forget(guid)` exists for this; nothing calls it automatically. `release_stale` (`:433`, 8 minutes) is what eventually clears the old entry — and **two board entries are what makes the deterministic engine engage**, so one stale entry turns a single ship into a sequencing problem against his own former self | `.. <cs> — nothing has accounted for him in 8 minutes, off the board`, and `released` records |
| **A clearance was refused** | Read the `CONTROLLER:` line — the engine, not the agent, wrote the refusal. The stock ones: `not radar identified, say your position and altitude` (`controller.py:979`, from `may_be_sequenced`, `:331` — on a vectored approach an unseen aircraft may not take a stack slot); `continue holding, number two, expect approach shortly` (`:1036`, the letdown block is occupied); `no holding available, remain clear` (`:789`) | If it is `not radar identified` while he is plainly on the scope, the `note_radar_contact` call at `:1758` resolved by callsign instead of track — that exact bug ran a whole sortie with the board reading UNSEEN |
| …refused with "negative, you are not over the beacon" on a **radar** approach | `separation_context` (`:1729`) rejects a claimed station passage the scope contradicts — but only when `beacon_flown`. The classifier files ordinary position reports as `REPORT_BEACON` because it is the nearest thing it knows | `!! rejected: claims the beacon, radar shows 8.2 nm`. If the profile is `vectored`, this guard should not have run at all |
| …vectored when he never asked, or vectored while holding | `may_be_vectored` (`:1530`) is the invariant as one question. It refuses if: he is not in `ctl.aircraft` (never asked for an approach), he is on a visual, he has not checked in **on this frequency** (`_heard_on`, `:1528`), or with traffic present he does not own the approach (`owns_the_approach()`) | Silence from the asr_monitor with a track plainly on final usually means `_heard_on` has him on a different channel |
| **Radio went silent mid-sortie** | In order of likelihood: the director wedged (a 30 s answer poisons the next calls — it returns `busy` and the bridge prints nothing); the bridge process died; the pilot is parked on the engineering line; the track went **stale** so radar reads `no contacts` and every radar-gated path switches off | `tools/bridge.py status`; `docker compose logs --tail=100`; `curl -s localhost:8000/radar`. `tracks.FRESH_SEC = 15` (`director/tools/tracks.py:120`) — a paused server stops the unit stream and a stale row reads as no-contact by design, never as confidently-wrong |
| …silent only on **one** frequency | The bridge answers on the channel the call arrived on (`voice_for(on_hz)`, `:2652`). A hook fires on `hook_frequency(why, _heard_on, …)` (`:2222`) — the channel we last heard *that man* on. If he changed radios after the promise, the callback goes to the old one | `HOOK fired (+60s) on 124.000:` — check that number against the frequency in his last `pilot` record |
| **The agent paraphrased instead of reading the numbers** | The two-brain seam. The `CONTROLLER:` / `SEPARATION:` / `ASR:` blocks are appended to the model's context with explicit "voice these exactly" wording (`:3495`–`:3520`); if the numbers on the air differ from the ones in those lines, it is the model | Confirm without flying: `uv run --extra voice python tools/atc_dryrun.py --script formation`. It assembles the *same* message the live loop does and prints `CONTROLLER:` beside the reply, so the drift is visible in seconds |
...
| `tools/engineer.py` + `/tmp/marshall-say` | Talk to the pilot mid-sortie without the controller hearing it. `echo "…" >> /tmp/marshall-say`. Mostly superseded — the bridge now speaks for engineering in the "Amy" voice — but the name stays in `OUR_STATIONS` (`:1815`) so an old one left running is never mistaken for a pilot |
...
- **`who`** — the handle (`identity.handle`, `identity.py:210`), the human out of the squadron name. It is what a formation split falls back to.
...
The `board` record written immediately after it (`:3184`) is the separation engine's state at that instant, deliberately adjacent — a ghost is created by a transmission, so the transmission and the board have to be next to each other or the pairing is guesswork later.
...
`to` is `addressed_to(reply)` (`:2187`) — who the words open by naming. When `to` and the preceding `pilot` record's `callsign` differ, the controller answered the right pilot by the wrong name, which is a different bug from answering the wrong pilot. `whos_who.py` flags that pair as `<-- MISMATCH`.
...
- **The engine is blind and cannot retract.** It believes position reports; the agent sees radar. A flight once called "over the beacon" at eight miles — the agent refused correctly, but the engine had *already* broken the formation up. The bridge reconciles the two at `:1725`, but only for a claimed station passage and only when a beacon is being flown.
- **`_flights`, `_heard_on`, `_seen_at`, `_transmitters`, `_awaiting_readback` and the missed-approach latch all live in bridge process memory.** A bridge restart forgets every one of them. `may_be_vectored` deliberately keys traffic off the *scope* rather than the stack for this reason (`:1553`) — radar does not forget over a restart.
