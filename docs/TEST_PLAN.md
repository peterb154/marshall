...
| **S10** | **Every clearance you are given** — taxi, take-off, approach, landing, a hold | The **numbers actually reach you.** The engine decides them and the agent phrases them; if it phrases them away, the bridge now appends the missing clause. On the recorded sortie *"runway one three, cleared for take-off"* was transmitted as **"Sockeye, roger."**, and a refusal naming Tower 133.0 came out as **"go ahead."** Report any clearance that arrives without its runway, level, station or frequency | [#79] | **P1** |
| **S11** | A repaired transmission (S10 firing) | It should sound like **one controller finishing his sentence**, not a template read out after a human. The agent's own words are kept and only the missing clause is added — if it sounds bolted on, or says the same thing twice, that is the finding. **This is the one thing on the card that cannot be checked offline at all** | [#79] | P2 |
