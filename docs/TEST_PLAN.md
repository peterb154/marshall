...
```rows
G4 G14 G21 G19 G12 G22 G23 G16 R21 V9 Q15 Q18 G15 Q16 V7 R15 R16 R17 R18 R20 N1 N2 M1 H10 V6 V3 H5 H6 H4 H9 H11 H29 H30 V8 V5 R19 H32 H33 H31 K6 K7
```
...
```rows
M2 M3 M4 Q1a R7 H10 H9 H4 H5 H6 H20 H21 H24 H26
```
...
| G22 | P2 | **Read the departure frequency back in a different idiom** — *"one two three point three"*, or *"1-2-3-3"*, or just *"123.3"* | Accepted. Only one spoken form verifies today, so a correct read-back in another idiom reads as missing and the correction asks for the number you just said. Low priority — it costs a transmission, not a sortie | [#214] |
| G23 | **P1** | **Say your departure runway on the ground and then go flying** — *"holding short, runway 7, ready for departure"*, and taxi and take-off calls that name it too. Then, airborne and inbound to the OTHER field, ask Approach for an approach | **You were never given one on the ground.** Saying the runway you are LEAVING FROM assigned an approach INTO the field you were leaving: on 29 August a two-ship was silently put on the Kobuleti ILS 07 while holding short at Kobuleti, every seat afterwards worked it as an arrival there, and the pilot found out forty minutes later inbound to Batumi when he could not change it — *"I didn't request rwy 7?"*. He had not. Check `/diag` on the ground shows **no** cleared approach, and that the one you ask for later is the one you get | [#177] |
...
| G19 | P1 | **Read the clearance back until he says "readback correct"** — then immediately ask Ground to taxi | You are cleared to taxi. What he SAID and what was RECORDED must be the same thing: on 28 August he said "readback correct" twice, `assigned_plans.acked_at` stayed NULL both times, Ground went on refusing taxi and the sortie was unwinnable. Check `/diag` reads the clearance as read back, not just the radio | [#185] |
| G21 | P1 | **Ask Clearance what you are cleared for BEFORE asking for anything**, then ask Ground the same question | **They agree.** On 28 August Clearance said *"already cleared on BatumiTest"* and Ground said *"you have not been cleared"* — for twenty minutes, with `assigned_plans` empty. `board.find` matched `callsign = %s` exactly, the row is bound `sockeye`, the agent asks for `Sockeye`, and every clearance tool answered as though he did not exist | [#210] |
| ~~G20~~ | P1 | **After he says "readback correct", keep talking** — report your position, or ask him anything at all | He does not restart the read-back. On 28 August the accumulator was discarded the moment the read-back was agreed, so the next transmission was judged as a fresh read-back of the WHOLE clearance and came back *negative* on two numbers already read back correctly — immediately after being told he was correct | [#208] |
...
| G16 | P1 | After the read-back, **look at the board**, then ask Ground and Tower which flight plan you are on | The board names it — `flight plan: BatumiTest`, the route, and `read back`. `cleared approach` is a SEPARATE row and being blank there is correct until Approach clears you. On 18 August the only clearance field said *"cleared for —"*, which reads as no clearance, and he taxied back to Clearance to check. Every seat should answer which plan you are on from its strip; none should send you to Clearance for it | [#191] |
| ~~G17~~ | P1 | **Open the sortie with an odd first call** — say your parking spot, or *"down and stopped"*, or *"clear of the active"*, before asking for anything | He works it. A canned *"roger, welcome, good day"* to an opening call means a regex answered and the engine never saw the transmission — that is exactly what happened on 18 August, where the word **parking** in *"parking spot, number 22 with information, Delta"* matched a closing-acknowledgement pattern. Then try a bare *"radio check"*: that one SHOULD come back instantly with *"loud and clear"*. And *"how do you read"* with your callsign must NOT — it is a check-in | [#194] |
| ~~G18~~ | P1 | **Cold and dark, having never flown**, open with *"down and stopped, spot 22"* — then look at the board | **Not LANDED.** The separation phase must stay where a parked aeroplane's belongs. On 28 August this set `Phase.LANDED` on the sortie's first transmission, and at take-off `phases._wanted` read that back as evidence of having flown — bypassing the `has_flown` protection #178 was built for — so the board went `landed` **after** the sim's own takeoff event and Departure posted him back to Tower for fourteen miles. Engineering should show a CONTROLLER ANOMALY naming the refusal | [#206] |
...
| **H31** | **On the roll-out after landing**, and again on a deliberate **go-around** | Rolling out you stay with Tower until he sends you to Ground — nothing may offer you Approach or Departure while you are decelerating on the runway (it did, fifteen seconds after touchdown). Then go around deliberately: that one MUST hand you back to Approach, because it is the case the branch was written for and the fix narrows it | [#200] | **P1** |
| **H32** | **Fly the ILS all the way down and count what he SAYS** — every turn and every altitude | You hear them. On 28 August the engine decided five vectors between twelve thousand and two thousand and **not one reached the air**: the ASR block tells the agent the turns are transmitted automatically, and nothing transmitted them. Check the recorder for `not_voiced` — six decided facts were lost that sortie and all six were the approach | [#79] | **P1** |
| **H33** | **Once cleared for the approach, note who works you** | Approach keeps you until Tower. On 28 August he was cleared for the ILS and twenty-five seconds later got *"radar service terminated, contact Georgia Center"* — handed backwards to the enroute controller while being vectored to intercept. Asking again got *"you remain cleared"*, so the engine had not let go: the handoff was the agent's | [#213] | **P1** |
...
| **R15** | **Ask for flight following** — "Georgia Center, Sockeye, request flight following to Batumi" — with a plan filed, and again with nothing filed at all, asking for **direct BAR** | Granted, and you get the first leg straight away: a heading, a distance and an altitude. Direct-to-a-fix works without a plan; a plan is only the longer route. If there is nothing to follow he says so rather than going quiet | [#217] | **P1** |
| **R16** | **Fly the whole ladder while receiving it** — Departure, Center, Approach | It **survives every handoff**, and the receiving controller says so: *"flight following continues"*. This is the half that spans seats and the half most likely to break. If a new controller goes quiet about it, that is the defect | [#217] | **P1** |
| **R17** | **Fly over a steerpoint**, then cut the corner on the next one rather than overflying it | Passage is called both times, once each, with the NEW heading, distance and altitude even when they are unchanged. Cutting the corner must still count — passage is the fix going behind you, not a radius you have to fly through | [#217] | **P1** |
| **R18** | **Drift three or four miles off a leg** and hold it, then come back | You are told, once, and not nagged on the boundary. Coming back stops it. What must NOT happen: an off-course call during the turn at a steerpoint — at 400 knots a ninety-degree turn swings four miles wide before it settles — or while you are flying a heading he just gave you | [#217] | **P1** |
| **R19** | **Cancel it** — "cancel flight following" — then ask again, then fly an approach | Two different endings and the right one each time: *"resume own navigation"* when the vectors stop, *"radar service terminated"* when the service does. Being cleared for an approach supersedes it and the leg calls stop | [#217] | **P2** |
| **R20** | **Say nothing about it all sortie** | You hear **nothing new**. Own nav is the default and silence is the whole point of it — in a combat sim the nag is the failure | [#217] | **P1** |
| **R21** | **In the Phantom, dial every channel on the UHF card** and speak on each | Somebody answers on all of them, and it is the right facility for the number. The map had no UHF at all until 31 August, so an ARC-164 could not reach a single controller. Check the ATIS too — Kobuleti 279.00, Batumi 280.00 | [#218] | **P1** |
...
| **V6** | **Fly the ILS all the way in from thirty miles**, and watch for any moment the controller says you have left his airspace or hands you somewhere unexpected | Nothing bounces. Batumi's terminal area now reaches 27.5 nm and its ILS holds at KOBULETI 22.5 nm out, so the whole procedure is inside the airspace that works it — where before the area stopped at 11.3 and the approach began at double that. Also: sitting on Kobuleti's ramp, you are Kobuleti's, never Batumi's | [#139] | **P1** |
| **V7** | **Recover from thirty miles out** and note where Georgia Center hands you to Approach. Then depart and note where Departure gives you to Center | Approach takes you around **27–28 nm**, comfortably before the procedure starts at 22 — not at a flat 25, and not at 11. Departure keeps you to about 29 nm outbound. **On the Kobuleti-to-Batumi hop you will get no enroute leg at all, and that is correct** — you turn for the destination long before you are 29 miles out. The comms card is what is wrong there, not the controller | [#130] | **P1** |
| **V8** | **On the ILS into Batumi, note which frequency the mile calls and the landing clearance arrive on.** Then have a second aircraft recovering to Kobuleti at the same time | Each of you hears his own final on his own controller's channel — never the other field's, and never a channel with nobody on it. One controller, one channel: the conversation, the vectors and the mile calls all from the same voice | [#173] | **P1** |
| **V9** | **Take off without waiting for a taxi or take-off clearance** — roll from the ramp and get airborne while the board still has you in the clearance phase | Nobody sends you to **Ground** once you are flying. Tower takes you back. An airborne aeroplane is never Ground's, and the board's phase must not be able to override that | [#168] | **P2** |
| **V10** | **On the diag board, with two aircraft worked by seats at DIFFERENT aerodromes, read the datum column on each row** — and leave it alone for a minute so the page refreshes on its own | Each row says the field of the seat working THAT aeroplane, and the range beside it is measured from there. Not one field for the whole page, and not the loaded approach's. A refresh with nobody talking must not change either | [#169] | **P2** |
| **V11** | **Fly the 1944 letdown** — start the radio on `batumi-ndb-12` and ask for a frequency, a handoff, anything with a number in it | He can NAME one. Every frequency he gives you is a beacon you can actually tune — 128.0, 132.0, 124.0 — never a modern channel like 118.6 that an ARA-8 cannot reach. Before this he could name none at all, and nobody had flown it to find out | [#140] | **P2** |
